...
null, missing values and empty objects remain distinct. `body_exact = true`
requires identical body bytes. There is no network fallback. Missing, extra
...
| Surface | Replay integration | Real capture status |
| --- | --- | --- |
...
| Anthropic Messages, thinking signatures, tools/results, cache usage, cancellation | `anthropic_http_spec.lua` | Synthetic; real Anthropic inference needed. |
