...
describe("neoagent.transport.sse", function()
  ...
  it("bounds an unterminated line", function()
    ...
    assert.is_nil(ok)
    local message = assert(err)
    assert.matches("exceeded", message)
    assert.is_nil((parser:feed("later")))
    local finished, finish_err = parser:finish()
    assert.is_nil(finished)
    assert.are.equal(message, finish_err)
    finished, finish_err = parser:finish()
    assert.is_nil(finished)
    assert.are.equal(message, finish_err)
  end)
  ...
  it("bounds a complete multiline event", function()
    ...
    assert.is_nil(ok)
    local message = assert(err)
    assert.matches("event exceeded", message)
  end)
