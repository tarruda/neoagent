...
describe("neoagent state store", function()
  ...
  it("recovers from missing and malformed documents", function()
    ...
    assert.are.equal("state_store", assert(err).kind)
    assert.matches("invalid JSON", assert(err).message)
  end)

  it("rejects invalid ids and entries", function()
    local value = store()
    assert.has_error(function() value:read(1 --[[@as string]]) end)
    assert.has_error(function() value:read("") end)
    ...
    assert.is_nil(result)
    assert.matches("entry must contain", assert(err).message)
  end)
  ...
  it("reports atomic write and cleanup failures", function()
    ...
    assert.is_nil(result)
    assert.are.equal("state_store", assert(err).kind)
    assert.matches("rename failed", assert(err).message)
    ...
    assert.is_nil(result)
    assert.are.equal("state_store", assert(err).kind)
    assert.is_true(#assert(err).message > 0)
  end)
  ...
  it("tolerates unusable state directories and reports failures through operations", function()
    ...
    assert.is_nil(entry)
    assert.are.equal("state_store", assert(read_err).kind)
    local result, err = value:write("entry", { models = {} })
    assert.is_nil(result)
    ...
    assert.is_nil(result)
    assert.matches("private state directory", assert(err).message)
    result, err = tolerated:delete("entry")
    assert.is_nil(result)
    assert.matches("private state directory", assert(err).message)
  ...
  it("reports real lock path conflicts for writes and deletes", function()
    local value = store()
    local write_lock = value:path("write-locked") .. ".lock"
    assert.are.equal(1, vim.fn.mkdir(write_lock, "p"))
    local written, write_err = value:write("write-locked", { value = true })
    assert.is_nil(written)
    assert.is_not_nil(write_err)

    local delete_lock = value:path("delete-locked") .. ".lock"
    assert.are.equal(1, vim.fn.mkdir(delete_lock, "p"))
    local deleted, delete_err = value:delete("delete-locked")
    assert.is_nil(deleted)
    assert.is_not_nil(delete_err)
  end)
end)
