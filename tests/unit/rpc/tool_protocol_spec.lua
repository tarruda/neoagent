...
describe("neoagent Tool RPC protocol", function()
  ...
  it("validates exact messages and copied call context", function()
    ...
    local value = protocol.validate({
      ...
      request_id = 1,
      method = "write_file",
      payload = { path = "a", resolved_path = "/workspace/a", content = "b" },
    })
    ...
    local response = protocol.validate({
      ...
      call_id = "call-1",
      request_id = 1,
      value = { content = { { type = "text", text = "denied" } }, isError = true },
    })
    local result = codec.result(response.value)
  ...
  it("rejects malformed Tool connection context", function()
  ...
  it("rejects marker mismatch and terminates the channel", function()
    ...
    local value = wait(async.run(function()
      remote:open(codec.encode_context(call()))
      return true
    end))
  ...
  it("rejects premature handshake and duplicate terminal events", function()
    local premature = require("neoagent.rpc.connection").new()
    ...
    local premature_result = wait(async.run(function()
      premature:open(codec.encode_context(call()))
      return true
    end))
    ...
    local duplicate = require("neoagent.rpc.connection").new()
    ...
    local duplicate_result = wait(async.run(function()
      duplicate:open(codec.encode_context(active_call))
      return tool_client.invoke(duplicate, "write_file", { path = "file", resolved_path = "/workspace/file", content = "content" }, active_call)
    end))
    assert.are.equal("done", duplicate_result.content[1].text)
    assert.are.equal("failed", duplicate._state)
    assert.are.equal("protocol", assert(duplicate._failure).kind)
    assert.is_false(pcall(duplicate.start_request, duplicate, "write_file", {}))
  ...
  it("preserves ordered updates and orderly shutdown", function()
    ...
    local value = wait(async.run(function()
      ...
      local result = tool_client.invoke(remote, "write_file", { path = "file", resolved_path = "/workspace/file", content = "content" }, active_call)
      remote:close()
      return result
    end))
  ...
  it("rejects parent-only Tool requests in the worker", function()
    local events = {}
    local worker = require("neoagent.rpc.server").new({
      send = function(message) events[#events + 1] = message end,
    })
    worker:receive({ type = "open", call_id = "parent-only", context = codec.encode_context(call()) })
    for index, method in ipairs({ "read_agent_documentation", "update_plan" }) do
      worker:receive({ type = "request", call_id = "parent-only", request_id = index,
        method = method, payload = {} })
      assert(vim.wait(1000, function()
        local event = events[#events]
        return event and event.type == "request_error" and event.request_id == index
      end))
      assert.matches("unknown Tool RPC method", events[#events].error.message, 1, true)
    end
    worker:eof()
    assert.is_true(worker:is_quiescent())
  end)
  ...
  it("fails skipped update sequences and callback failures", function()
    for _, scenario in ipairs({ "sequence", "callback" }) do
      ...
      local value = wait(async.run(function()
        remote:open(codec.encode_context(active_call))
        return tool_client.invoke(remote, "write_file", { path = "file", resolved_path = "/workspace/file", content = "content" }, active_call)
      end))
      ...
      local child, state = fake_child(remote, function(message, emit)
        ...
        elseif message.type == "request" then
          emit({
            ...
            value = { content = { { type = "text", text = "stale" } } },
          })
        end
      end)
      remote:attach(child)
  ...
  it("pins workspace authority in the fixed Tool proxy", function()
    ...
    assert.is_false(value.ok)
    assert.matches("Tool workspace changed", value.error.message)
  ...
  it("classifies worker exits without taking over lease disposal", function()
    local before_ready = require("neoagent.rpc.connection").new()
    ...
    local starting = async.run(function()
      before_ready:open(codec.encode_context(call()))
      return true
    end)
    ...
    local active = require("neoagent.rpc.connection").new()
    ...
    local pending = async.run(function()
      active:open(codec.encode_context(active_call))
      return tool_client.invoke(active, "write_file", { path = "file", resolved_path = "/workspace/file", content = "content" }, active_call)
    end)
    ...
    local settled = require("neoagent.rpc.connection").new()
    ...
    local settled_result = wait(async.run(function()
      settled:open(codec.encode_context(settled_call))
      return tool_client.invoke(settled, "write_file", { path = "file", resolved_path = "/workspace/file", content = "content" }, settled_call)
    end))
  ...
  it("bounds worker startup and orderly shutdown waits", function()
    local starting = require("neoagent.rpc.connection").new({ start_grace_ms = 10 })
    ...
    local closing = require("neoagent.rpc.connection").new({ shutdown_grace_ms = 10 })
    ...
    local closing_result = wait(async.run(function()
      closing:open(codec.encode_context(call()))
  ...
  it("propagates cancellation through request and close acknowledgements", function()
    ...
    local run = async.run(function()
      remote:open(codec.encode_context(active_call))
      return tool_client.invoke(remote, "shell", { argv = { "sh", "-c", "wait" } }, active_call)
    end)
...
describe("neoagent Tool RPC server", function()
  it("dispatches fixed methods and emits monotonic updates", function()
    ...
    server:receive({
      ...
      request_id = 1,
      method = "write_file",
      payload = { path = "file", resolved_path = "/workspace/file", content = "content" },
    })
    assert(vim.wait(1000, function()
