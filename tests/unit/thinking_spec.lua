...
describe("neoagent thinking levels", function()
  ...
  it("returns independent request options and handles unsupported models", function()
    ...
    assert.is_true(thinking.is_level("ultra"))
    assert.is_false(thinking.is_level("other"))
  end)

  it("orders ultra above max", function()
    local strongest = { thinking = { max = {}, ultra = {} } }
    assert.are.same({ "max", "ultra" }, thinking.levels(strongest))
    assert.are.equal("ultra", thinking.clamp(strongest, "ultra"))
    assert.are.equal("ultra", thinking.next(strongest, "max"))
  end)
end)
