...
describe("neoagent.session", function()
  ...
  it("clears the active leaf when returning to the empty branch", function()
    local session = assert(Session.new())
    assert(session:append({ role = "user", content = "one" }))
    assert(session:move_to(nil))
    assert.is_nil(session:leaf_id())
    assert.are.same({}, assert(session:context_messages()))
    local snapshot = assert(session:snapshot())
    assert.is_nil(snapshot.leaf_id)
    local reopened = assert(Session.new({ entries = snapshot.entries }))
