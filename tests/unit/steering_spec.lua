...
describe("neoagent Agent steering ownership", function()
  it("offers, acknowledges, claims, and restores records by identity", function()
    ...
    local pending_claim, pending_err = queue:claim(second.id)
    assert.is_nil(pending_claim)
    assert.matches("acknowledgement is pending", assert(pending_err).message)
