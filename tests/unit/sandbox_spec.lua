...
local function test_platform(on_start)
  return {
    ...
    exec = function()
      error("unexpected completion-oriented sandbox execution")
    end,
...
describe("neoagent sandbox composition", function()
  ...
  it("blocks unavailable sandbox execution until explicitly disabled",
    ...
    local execute = assert(direct.execute_tool)
    local direct_blocked = complete(function()
      return execute(assert(assert(direct.tools)[1]), {}, context(root))
    end)
    assert.is_true(direct_blocked.isError)
    assert.are.equal(1, executions)

    local parent_tool = util.copy(tool)
    parent_tool.name = "parent-inspect"
    local parent = composition.agent(require("neoagent.config").resolve({
      tools = { parent_tool }, sandbox = { enabled = true, parent_tools = { parent_tool } },
    }), options)
