...
describe("OpenCode Go Qwen missing-tool recovery", function()
  ...
  it("continues the recorded thinking-only tool-use response through authenticated requests", function()
    ...
    assert.are.equal("I will inspect the condition.", assert(assert(result.message).content[2]).thinking)
    assert.are.equal("inspect", assert(assert(result.message).content[3]).name)
    assert.are.same({ path = "example.lua" }, assert(assert(result.message).content[3]).arguments)
    assert.are.equal(3970, assert(assert(result.message).usage).input)
    assert.are.equal(365, assert(assert(result.message).usage).output)
    assert.are.equal(81920, assert(assert(result.message).usage).cacheRead)
    ...
    local second = vim.json.decode((assert(assert(assert(player).requests[2]).body)))
    ...
    assert.are.equal("recovery-session", rawget(assert(assert(assert(player).requests[2]).headers), "x-opencode-session"))
  ...
  it("stops after one unsuccessful follow-up and retains both responses", function()
    ...
    assert.are.equal(2, #assert(result.message).content)
    assert.are.equal(thought, assert(assert(result.message).content[1]).thinking)
    assert.are.equal(thought, assert(assert(result.message).content[2]).thinking)
    assert.are.equal(7900, assert(assert(result.message).usage).input)
    assert.are.equal(1, warnings)
