...
---@param lines string[]
---@param value string
local function contains(lines, value)
...
describe("neoagent UI mappings", function()
  it("opens full details for the transcript card under the cursor", function()
    ...
    assert(vim.wait(1000, function()
      ...
      return contains(lines, "5 more lines")
    end))
    result:focus_transcript()

    local transcript = vim.api.nvim_buf_get_lines((assert(view_handles.buffer(result, "transcript"))), 0, -1, false)
    ...
    assert(vim.wait(1000, function()
      return view_handles.window(result, "details") and vim.api.nvim_win_is_valid((assert(view_handles.window(result, "details"))))
    end))
    assert.matches("Text", vim.api.nvim_win_get_config((assert(view_handles.window(result, "details")))).title[1][1])
    ...
    vim.api.nvim_win_set_cursor((assert(view_handles.window(result, "transcript"))), { shell_row, 0 })
    vim.api.nvim_feedkeys(vim.api.nvim_replace_termcodes("<CR>", true, false, true), "x", false)
    assert(vim.wait(1000, function()
      return view_handles.window(result, "details") and vim.api.nvim_win_is_valid((assert(view_handles.window(result, "details"))))
    end))
  ...
  it("preserves the transcript view while card details are open", function()
    ...
    assert(result:open())
    assert(vim.wait(1000, function()
      return contains(vim.api.nvim_buf_get_lines(
        (assert(view_handles.buffer(result, "transcript"))), 0, -1, false), "card 30")
    end))
    result:focus_transcript()
    ...
    vim.api.nvim_win_set_cursor((assert(view_handles.window(result, "transcript"))), { target, 0 })
    ...
    local view = vim.api.nvim_win_call(
      (assert(view_handles.window(result, "transcript"))), function() return vim.fn.winsaveview() end)
  ...
  it("centers the expanded card in the transcript without moving focus", function()
    ...
    vim.api.nvim_win_set_cursor((assert(transcript_window)), { card_row, 0 })
    vim.api.nvim_feedkeys(
      vim.api.nvim_replace_termcodes("<CR>", true, false, true), "x", false)
    assert(vim.wait(1000, function()
      return view_handles.window(result, "details")
        and vim.api.nvim_win_is_valid((assert(view_handles.window(result, "details"))))
    end))
    local details_window = view_handles.window(result, "details")
    vim.api.nvim_win_call((assert(transcript_window)), function()
      vim.cmd("normal! Gzt")
    end)
    local before = vim.api.nvim_win_call((assert(transcript_window)), function()
    ...
    assert(vim.wait(1000, function()
      ...
      local target = block and assert(result.transcript.pane.layout).targets[
        "card:" .. block.key]
      return target and vim.api.nvim_win_get_cursor((assert(transcript_window)))[1]
        == target.point.row + 1
    end))
    assert.are.equal(details_window, vim.api.nvim_get_current_win())
    local cursor = vim.api.nvim_win_get_cursor((assert(transcript_window)))[1]
    local visible = vim.api.nvim_win_call((assert(transcript_window)), function()
      return { vim.fn.line("w0"), vim.fn.line("w$") }
    end)
    assert.is_true(math.abs(assert(cursor) - (visible[1] + visible[2]) / 2) <= 1)
    result:destroy()
  end)
  ...
  it("toggles raw Markdown for text and thinking details only", function()
    ...
    local function title()
      local chunks = vim.api.nvim_win_get_config((assert(view_handles.window(result, "details")))).title or {}
    ...
    local function check_toggle(row, raw)
      ...
      feed("r")
      assert(vim.wait(1000, function() return assert(result.details).raw end))
      assert(assert(result.details).pane:flush())
    ...
    vim.api.nvim_win_set_cursor((assert(view_handles.window(result, "transcript"))), { rows.tool, 0 })
    feed("<CR>")
    assert(vim.wait(1000, function()
      return view_handles.window(result, "details") and vim.api.nvim_win_is_valid((assert(view_handles.window(result, "details"))))
    end))
    vim.api.nvim_buf_call((assert(view_handles.buffer(result, "details"))), function()
  ...
  it("toggles periodic following for expanded transcript cards", function()
    ...
    assert.is_true(result:show_card_details(assert(block).key))

    local window = view_handles.window(result, "details")
    local buffer = view_handles.buffer(result, "details")
    local function title()
      local chunks = vim.api.nvim_win_get_config((assert(window))).title or {}
    ...
    local function at_end()
      local last = vim.api.nvim_buf_line_count((assert(buffer)))
      return vim.api.nvim_win_get_cursor((assert(window)))[1] == last
        and vim.api.nvim_win_call((assert(window)),
          function() return vim.fn.line("w$") end) == last
    end

    assert.is_true(vim.api.nvim_buf_line_count((assert(buffer)))
      > vim.api.nvim_win_get_height((assert(window))))
    ...
    assert.are.same({ 1, 0 }, vim.api.nvim_win_get_cursor((assert(window))))
    ...
    assert(vim.wait(1000, function()
      local mapped = vim.api.nvim_buf_call(
        (assert(buffer)), function() return vim.fn.maparg("<A-f>", "n") end)
  ...
  it("navigates vertically between transcript cards and input across modes", function()
    ...
    assert(vim.wait(1000, function()
      return contains(vim.api.nvim_buf_get_lines(
        (assert(view_handles.buffer(result, "transcript"))), 0, -1, false), "third card")
    end))
    result:focus_transcript()
    ...
    local function cursor_row()
      return vim.api.nvim_win_get_cursor((assert(view_handles.window(result, "transcript"))))[1]
    end

    vim.api.nvim_win_set_cursor((assert(view_handles.window(result, "transcript"))), { rows["between cards"], 0 })
    feed("<A-j>")
    ...
    feed("<A-k>")
    assert.are.equal(rows["first card"], cursor_row())
    vim.api.nvim_win_set_cursor((assert(view_handles.window(result, "transcript"))), { rows["third card"], 0 })
    feed("<A-j>")
    assert(vim.wait(1000, function()
  ...
  it("reveals the complete last card after leaving a scrolled transcript", function()
    ---@param scroll_on_transcript_leave boolean
    ...
    local messages = {}
      ...
      assert(vim.wait(1000, function()
        return contains(vim.api.nvim_buf_get_lines(
          (assert(view_handles.buffer(result, "transcript"))), 0, -1, false), "last card three")
      end))
      ...
      local function visible_rows()
        return vim.api.nvim_win_call((assert(window)), function()
          return { vim.fn.line("w0"), vim.fn.line("w$") }
        end)
      end
      ---@param keys string
    local function feed(keys)
      ...
      feed("<A-k>")
      assert.is_true(vim.api.nvim_win_get_cursor((assert(window)))[1] < first_row)
      result:focus_input()
      ...
      assert.are.equal(target_row,
        vim.api.nvim_win_get_cursor((assert(window)))[1])
  ...
  it("moves from the transcript document bottom into the input", function()
    ...
    assert(vim.wait(1000, function()
      return contains(vim.api.nvim_buf_get_lines(
        (assert(view_handles.buffer(result, "transcript"))), 0, -1, false), "last card")
    end))
    result:focus_transcript()

    vim.api.nvim_feedkeys("G", "x", false)
    assert.are.equal(vim.api.nvim_buf_line_count(
      (assert(view_handles.buffer(result, "transcript")))),
  ...
  it("moves directly between the transcript and input windows", function()
    ...
    assert(vim.wait(1000, function()
      return contains(vim.api.nvim_buf_get_lines(
        (assert(view_handles.buffer(result, "transcript"))), 0, -1, false), "transcript card")
    end))
  ...
  it("navigates between cards while their details own focus", function()
    ...
    vim.api.nvim_win_set_cursor((assert(view_handles.window(result, "transcript"))), { second_row, 0 })
    ---@param keys string
    local function feed(keys)
      vim.api.nvim_feedkeys(
        vim.api.nvim_replace_termcodes(keys, true, false, true), "x", false)
    ...
    local function details_contain(value)
      if not view_handles.buffer(result, "details") or not vim.api.nvim_buf_is_valid((assert(view_handles.buffer(result, "details")))) then
        return false
      end
  ...
  it("uses native Ctrl-J for newlines without submitting", function()
    ...
    assert(result:open())
    vim.api.nvim_buf_call((assert(view_handles.buffer(result, "input"))), function()
  ...
  it("animates the transcript footer without mutating yankable text", function()
    ...
    local lines = vim.api.nvim_buf_get_lines((assert(view_handles.buffer(result, "transcript"))), 0, -1, false)
    local changedtick = vim.api.nvim_buf_get_changedtick((assert(view_handles.buffer(result, "transcript"))))
    local first = footer()
    assert(vim.wait(1000, function() return footer() ~= first end))
    assert.are.same(lines, vim.api.nvim_buf_get_lines((assert(view_handles.buffer(result, "transcript"))), 0, -1, false))
    assert.are.equal(changedtick, vim.api.nvim_buf_get_changedtick((assert(view_handles.buffer(result, "transcript")))))
  ...
  it("defers streaming while an input yank is operator-pending", function()
    ...
    vim.cmd("stopinsert")
    vim.api.nvim_win_set_cursor((assert(view_handles.window(result, "input"))), { 1, 0 })
    vim.fn.setreg("+", "sentinel")
    ...
    assert.are.equal(changedtick,
      vim.api.nvim_buf_get_changedtick((assert(view_handles.buffer(result, "transcript")))))
    assert.matches("o", vim.fn.state("oS"))
    assert.are.equal("+", vim.v.register)
    assert.are.equal("y", vim.v.operator)
  ...
  it("yanks an input Visual selection to the clipboard while streaming", function()
    ...
    local cursor = vim.api.nvim_win_get_cursor((assert(view_handles.window(result, "input"))))
    local anchor = vim.fn.getpos("v")
    local lines = vim.api.nvim_buf_get_lines((assert(view_handles.buffer(result, "transcript"))), 0, -1, false)
    local changedtick = vim.api.nvim_buf_get_changedtick((assert(view_handles.buffer(result, "transcript"))))
    local footer = vim.deepcopy(vim.api.nvim_win_get_config((assert(view_handles.window(result, "transcript")))).footer)
    ...
    assert.are.same(cursor, vim.api.nvim_win_get_cursor((assert(view_handles.window(result, "input")))))
    assert.are.same(anchor, vim.fn.getpos("v"))
    assert.are.same(lines, vim.api.nvim_buf_get_lines((assert(view_handles.buffer(result, "transcript"))), 0, -1, false))
    assert.are.equal(changedtick, vim.api.nvim_buf_get_changedtick((assert(view_handles.buffer(result, "transcript")))))
    assert.are.same(footer, vim.api.nvim_win_get_config((assert(view_handles.window(result, "transcript")))).footer)
  ...
  it("browses multiline input history and opens history selection", function()
    ...
    vim.cmd("stopinsert")
    result:set_input("draft")
    vim.api.nvim_win_set_cursor((assert(view_handles.window(result, "input"))), { 1, 5 })
    ...
    vim.cmd("stopinsert")
    vim.api.nvim_buf_set_text((assert(view_handles.buffer(result, "input"))), 0, 6, 0, 6, { "!" })
    ...
    result:set_input("first\nsecond")
    vim.api.nvim_win_set_cursor((assert(view_handles.window(result, "input"))), { 2, 3 })
    vim.api.nvim_feedkeys(vim.api.nvim_replace_termcodes("i<Up>", true, false, true), "x", false)
    assert(vim.wait(1000, function()
      return vim.api.nvim_win_get_cursor((assert(view_handles.window(result, "input"))))[1] == 1
        and result:get_input() == "first\nsecond"
    end))
    vim.api.nvim_win_set_cursor((assert(view_handles.window(result, "input"))), { 2, 0 })
    vim.api.nvim_feedkeys(vim.api.nvim_replace_termcodes("i<Down>", true, false, true), "x", false)
    assert(vim.wait(1000, function()
      local cursor = vim.api.nvim_win_get_cursor((assert(view_handles.window(result, "input"))))
