...
describe("neoagent workspace trust UI", function()
  ...
  it("retains the draft after cancellation and persists a later decision", function()
    ...
    assert(vim.wait(1000, function() return not applet:is_open() end, 5))
    assert.are.equal("keep me", agent:applet():get_input())
    assert.are.equal("keep me", agent:applet():pending_message())
    assert.is_nil(vim.uv.fs_stat(trust_path))

    assert(applet:open())
    active_view = wait_for_dialog(agent)
    ...
    assert.are.equal("keep me", agent:get_session():messages()[1].content)
  end)
