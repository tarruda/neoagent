...
describe("neoagent default controller", function()
  ...
  it("toggles built-in sandbox execution while Chat or Neo is active", function()
    ...
    local ok, err = pcall(function()
      ...
      assert.is_true(unchanged.enabled)
  ...
  it("selects named and forked sessions by recent tree activity", function()
    ...
    vim.ui.select = function(items, options, callback)
      ...
      assert.matches("^● Parent%s+3d$", options.format_item(items[1]))
      assert.matches("^  ├─ Child%s+1h$", options.format_item(items[2]))
      assert.matches("^  │  └─ Grandchild%s+10m$", options.format_item(items[3]))
      assert.matches("^  └─ Sibling%s+30m$", options.format_item(items[4]))
      assert.matches("^  Recent root%s+2h$", options.format_item(items[5]))
      assert.matches("^  Weekly root%s+1w$", options.format_item(items[6]))
      assert.matches("^  Monthly root%s+2mo$", options.format_item(items[7]))
      assert.matches("^  Yearly root%s+2y$", options.format_item(items[8]))
      callback(items[2])
    end
