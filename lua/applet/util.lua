...
local function equal(left, right, seen)
  ...
  if kind ~= type(right) or kind ~= "table" then
    return false
  end
...
local function utf8_length(text, index)
  ...
  if
    first >= 0xE0
    and first <= 0xEF
    and continuation(second)
    and continuation(third)
...
function M.byte_col(text, display_col)
  ...
  if not text:find("[\128-\255]") then
    return math.min(display_col, #text)
  end
  ...
  for index = 0, vim.fn.strchars(text, 1) - 1 do
    ...
    if width + character_width > display_col then
      break
    end
