...
function M.floating(opts)
  opts = opts or {}
  applet_expect(type(opts) == "table", "Applet floating Host", "options must be a table", 3)
  ...
  applet_expect(
    containers[result.container] == true,
    "Applet floating Host.container",
    "must be editor, largest_window, or auto",
    3
  )
  applet_expect(
    sides[result.side] == true,
    "Applet floating Host.side",
    "must be left, right, top, bottom, or center",
    3
  )
  dimension(result.width, "Applet floating Host.width")
  dimension(result.height, "Applet floating Host.height")
  applet_expect(
    finite(result.margin) and result.margin >= 0 and result.margin % 1 == 0,
    "Applet floating Host.margin",
    "must be a non-negative integral cell count",
    3
  )
  applet_expect(
    finite(result.base_zindex) and result.base_zindex >= 1 and result.base_zindex % 1 == 0,
...
function M.tab(opts)
  opts = opts or {}
  applet_expect(type(opts) == "table", "Applet tab Host", "options must be a table", 3)
  ...
  applet_expect(
    tab_positions[result.position] == true,
    "Applet tab Host.position",
    "must be before, after, first, or last",
    3
  )
  applet_expect(util.nonempty_string(result.label), "Applet tab Host.label", "must be a non-empty string", 3)
  return result
end
...
function M.validate(value)
  ...
  if value.kind == "floating" then
    return M.floating(value)
  end
  if value.kind == "tab" then
    return M.tab(value)
  end
