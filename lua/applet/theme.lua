...
function Palette:group(name)
  applet_expect(util.nonempty_string(name), "palette group",
...
function Palette:terminal(index)
  applet_expect(type(index) == "number" and index >= 0 and index <= 255
...
function Palette:blend(background, foreground, alpha)
  ...
  applet_expect(bottom_color ~= nil and top_color ~= nil, "palette blend",
    "requires resolvable colors", 3)
  applet_expect(finite(alpha) and alpha >= 0 and alpha <= 1,
...
local function normalized_highlight(palette, spec)
  ...
  if spec.base ~= nil then
    applet_expect(util.nonempty_string(spec.base), "highlight.base",
...
function Theme.new(opts)
  ...
  applet_expect(opts.name == nil or util.nonempty_string(opts.name),
    "theme.name", "must be a non-empty string", 3)
  applet_expect(opts.groups == nil or type(opts.groups) == "table",
    "theme.groups", "must be a table", 3)
  applet_expect(opts.highlights == nil or type(opts.highlights) == "table"
      or type(opts.highlights) == "function",
    "theme.highlights", "must be a table or function", 3)
  applet_expect(opts.max_derived_highlights == nil
...
function Theme:_definitions()
  ...
  applet_expect(value == nil or type(value) == "table", "theme.highlights",
...
function Theme:derive(key, spec)
  applet_expect(util.nonempty_string(key), "derived highlight key",
    "must be a non-empty string", 3)
  applet_expect(type(spec) == "table", "derived highlight",
    "must be a table", 3)
