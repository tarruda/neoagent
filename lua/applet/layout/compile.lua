...
local function fields(value, accepted, path)
  ...
  for key in pairs(value) do
    applet_expect(accepted[key] == true, path .. "." .. tostring(key), "is not a recognized field", 4)
  end
end
...
local function copy_value(value, path, active)
  ...
  applet_expect(kind == "table", path, "must contain only plain data values", 4)
  applet_expect(getmetatable(value) == nil, path, "must be a plain table", 4)
  ...
  for key, item in pairs(value) do
    ...
    applet_expect(key_kind == "string" or key_kind == "number", path, "must use string or number keys", 4)
...
local function dimension_signature(value)
  ...
  return table.concat(
    { "content", tostring(value.content), "min", tostring(value.min), "max", tostring(value.max) },
    ":"
  )
end
...
local function validate_scalar_dimension(value, path, allow_zero)
  applet_expect(
    finite(value) and (allow_zero and value >= 0 or value > 0),
    path,
    allow_zero and "must be a non-negative finite number" or "must be a positive finite number",
    4
  )
  applet_expect(value <= 1 or value % 1 == 0, path, "must be a fraction in (0, 1] or an integral cell count", 4)
  return value
end
...
local function validate_dimension(value, path, content)
  ...
  applet_expect(content and type(value) == "table", path, "must be a number or content request", 4)
  fields(value, content_dimension_fields, path)
  applet_expect(
    value.content == true or (finite(value.content) and value.content >= 0 and value.content % 1 == 0),
    path .. ".content",
    "must be true or a non-negative integral cell count",
    4
  )
  if value.min ~= nil then
    validate_scalar_dimension(value.min, path .. ".min")
  end
  if value.max ~= nil then
    validate_scalar_dimension(value.max, path .. ".max")
  end
  return {
...
local function resolve_scalar(value, total)
  if value <= 1 then
    return math.max(1, math.floor(total * value + 0.5))
  end
...
local function border_metrics(border)
  ...
  if type(border) == "string" then
    applet_expect(
      borders[border] == true,
      "Pane.window.border",
      "must be a recognized border name or plain border table",
      4
    )
    return { top = 1, right = 1, bottom = 1, left = 1 }
  end
  applet_expect(
    type(border) == "table" and getmetatable(border) == nil,
    "Pane.window.border",
    "must be a string or plain border table",
    4
  )
  applet_expect(#border == 8, "Pane.window.border", "border tables must contain eight cells", 4)
  local count = 0
  for _ in pairs(border) do
    count = count + 1
  end
  applet_expect(count == 8, "Pane.window.border", "border tables must contain a dense list of eight cells", 4)
  local occupied = {}
  for index, cell in ipairs(border) do
    if type(cell) == "table" then
      applet_expect(
        getmetatable(cell) == nil and #cell >= 1 and #cell <= 2,
        "Pane.window.border." .. index,
        "must be a string or { text, highlight }",
        4
      )
      local cell_count = 0
      ...
      applet_expect(cell_count == #cell, "Pane.window.border." .. index, "must be a dense border cell", 4)
      applet_expect(
        cell[2] == nil or type(cell[2]) == "string",
        "Pane.window.border." .. index .. ".2",
        "highlight must be a string",
        4
      )
      cell = cell[1]
    end
    applet_expect(type(cell) == "string", "Pane.window.border." .. index, "must be a string or { text, highlight }", 4)
    applet_expect(
      util.display_width(cell) <= 1,
      "Pane.window.border." .. index,
      "must occupy at most one display cell",
      4
    )
    occupied[index] = cell ~= ""
  end
...
local function normalize_rect(value, path)
  ...
  applet_expect(integer(result.row) and integer(result.col), path, "row and col must be integral cells", 4)
  applet_expect(
    integer(result.width) and result.width > 0 and integer(result.height) and result.height > 0,
    path,
    "width and height must be positive integral cells",
    4
  )
  return result
end
...
local function host_bounds(host, editor, container)
  if host.kind == "tab" then
    return copy_rect(editor), copy_rect(editor)
  end
  ...
  applet_expect(
    available_width > 0 and available_height > 0,
    "Applet floating Host",
    "margin leaves no available cells",
    4
  )
  ...
  applet_expect(width > 0 and height > 0, "Applet floating Host", "resolved bounds must contain cells", 4)
...
function M.environment(opts)
  applet_expect(type(opts) == "table", "Applet environment", "options must be a table", 3)
...
local function validate_options(value, path, reserved)
  ...
  applet_expect(type(value) == "table" and getmetatable(value) == nil, path, "must be a plain table", 4)
  local result = copy_value(value, path)
  for option in pairs(result) do
    applet_expect(not (reserved and reserved[option]), path .. "." .. tostring(option), "is owned by Applet", 4)
  end
  return result
end
...
local function normalize_action(value, path, ctx)
  ...
  applet_expect(
    applet_actions[name] or ctx.handlers[name] or (ctx.has_action and ctx.has_action(name)),
    path .. ".action",
    "is unknown: " .. string.format("%q", name),
    4
  )
...
local function normalize_bindings(value, path, ctx)
  ...
  applet_expect(type(value) == "table" and getmetatable(value) == nil, path, "must be a plain list", 4)
  ...
  for index, binding in ipairs(value) do
    ...
    applet_expect(type(binding.action) == "table", item_path .. ".action", "must be an action", 4)
    local pair = mode .. "\0" .. lhs
    applet_expect(not seen_pairs[pair], item_path, "duplicates an equal-precedence mode and lhs", 4)
    ...
    applet_expect(binding.desc == nil or type(binding.desc) == "string", item_path .. ".desc", "must be a string", 4)
    applet_expect(
      binding.nowait == nil or type(binding.nowait) == "boolean",
      item_path .. ".nowait",
      "must be a boolean",
      4
    )
    applet_expect(
      binding.silent == nil or type(binding.silent) == "boolean",
      item_path .. ".silent",
      "must be a boolean",
      4
    )
...
local function measured_content(ctx, node, axis)
  ...
  if node.type == "split" then
    ...
    for _, item in ipairs(node.children or {}) do
      local child = measured_content(ctx, item.child, axis) + child_decoration(ctx, item.child, axis)
      if node.axis == axis then
        measured = measured + child
      else
        measured = math.max(measured, child)
      end
    end
    return measured
  end
...
local function chrome_metrics(ctx, key, border)
  if ctx.host.kind == "floating" then
    return border_metrics(border)
  end
...
local function resolve_content_dimension(value, total, node, axis, ctx)
  if type(value) == "number" then
    return resolve_scalar(value, total)
  end
  local content = value.content == true and measured_content(ctx, node, axis) or value.content
  local result = content + child_decoration(ctx, node, axis)
  if value.min ~= nil then
    result = math.max(result, resolve_scalar(value.min, total))
  end
  if value.max ~= nil then
    result = math.min(result, resolve_scalar(value.max, total))
  end
  return result
end
...
local function weighted_change(items, amount, growing)
  ...
  for index, item in ipairs(items) do
    local capacity = growing and (item.max or item.size + amount) - item.size or item.size - item.min
...
local function allocate(children, total, axis, ctx, path)
  ...
  for index, value in ipairs(children) do
    ...
    applet_expect(type(value.child) == "table", item_path .. ".child", "must be a layout node", 4)
    local basis = value.basis
    if basis ~= nil then
      basis = validate_dimension(basis, item_path .. ".basis", true)
    end
    ...
    local size = basis and resolve_content_dimension(basis, total, value.child, axis, ctx) or minimum
    size = math.max(minimum, maximum and math.min(maximum, size) or size)
    local grow = value.grow
    if grow == nil then
      grow = basis == nil and 1 or 0
    end
    local shrink = value.shrink == nil and 1 or value.shrink
    applet_expect(finite(grow) and grow >= 0, item_path .. ".grow", "must be a non-negative finite number", 4)
    applet_expect(finite(shrink) and shrink >= 0, item_path .. ".shrink", "must be a non-negative finite number", 4)
    items[#items + 1] = {
      key = value.key,
      node = value.child,
      basis = basis,
      ...
      signature = table.concat(
        {
          value.key,
          dimension_signature(basis),
          tostring(grow),
          tostring(shrink),
          tostring(declared_min),
          tostring(declared_max),
        },
        "\0"
      ),
    }
  ...
  if used < total then
    ...
    applet_expect(changed == remaining, path, "maximum constraints leave unallocated cells", 4)
  elseif used > total then
    ...
    applet_expect(
      changed == needed,
      path,
      "minimum constraints do not fit the available cells (" .. used .. " required, " .. total .. " available)",
      4
    )
  end
  return items
...
local function normalize_buffer(value, key, lifecycle, path)
  ...
  applet_expect(value.uri == nil or util.nonempty_string(value.uri), path .. ".uri", "must be a non-empty string", 4)
  applet_expect(value.filetype == nil or type(value.filetype) == "string", path .. ".filetype", "must be a string", 4)
  applet_expect(
    value.sensitive == nil or type(value.sensitive) == "boolean",
    path .. ".sensitive",
    "must be a boolean",
    4
  )
  applet_expect(not value.sensitive or lifecycle == "transient", path .. ".sensitive", "requires a transient Pane", 4)
  local options = validate_options(value.options, path .. ".options", reserved_buffer_options)
  if value.sensitive then
    applet_expect(
      options.swapfile == nil or options.swapfile == false,
      path .. ".options.swapfile",
      "must be false for a sensitive Pane",
      4
    )
    applet_expect(
      options.undofile == nil or options.undofile == false,
      path .. ".options.undofile",
      "must be false for a sensitive Pane",
      4
    )
...
local function normalize_window(value, path)
  ...
  return {
    ...
    options = validate_options(value.options, path .. ".options", reserved_window_options),
    host_options = {
      floating = validate_options(host_options.floating, path .. ".host_options.floating", reserved_window_options),
      tab = validate_options(host_options.tab, path .. ".host_options.tab", reserved_window_options),
    },
  }
end
...
local function normalize_pane_focus(value, pane, path)
  ...
  applet_expect(
    mode == "normal" or mode == "insert" or mode == "preserve",
    path .. ".mode",
    "must be normal, insert, or preserve",
    4
  )
  applet_expect(
    not ((mode == "insert" or mode == "preserve") and not editable),
    path .. ".mode",
    "requires an editable Pane",
    4
  )
  local cursor = value.cursor or "preserve"
  applet_expect(
    cursor == "preserve" or cursor == "start" or cursor == "end",
    path .. ".cursor",
    "must be preserve, start, or end",
    4
  )
...
local function project_mount(ctx, node, rect, state, path)
  ...
  applet_expect(
    not ctx.mounted_panes[pane],
    path .. ".pane",
    "is already mounted as " .. tostring(ctx.mounted_panes[pane]),
    4
  )
  ctx.mounted_panes[pane] = key
  local lifecycle = node.lifecycle or "retained"
  applet_expect(
    lifecycle == "retained" or lifecycle == "transient",
    path .. ".lifecycle",
    "must be retained or transient",
    4
  )
  applet_expect(
    node.owns_pane == nil or type(node.owns_pane) == "boolean",
    path .. ".owns_pane",
    "must be a boolean",
    4
  )
  applet_expect(node.required == nil or type(node.required) == "boolean", path .. ".required", "must be a boolean", 4)
  applet_expect(
    node.mount_revision == nil or type(node.mount_revision) == "string" or finite(node.mount_revision),
    path .. ".mount_revision",
    "must be a finite number or string",
    4
  )
  ...
  applet_expect(
    content.width > 0 and content.height > 0,
    path,
    "decoration leaves no positive Pane content rectangle",
    4
  )
  local scopes = {}
  for _, scope in ipairs(ctx.root_scopes) do
  ...
  local projection_kind = (state.layer ~= nil or ctx.host.kind == "floating") and "floating" or "split"
  local projection
...
local function project_scope(ctx, node, rect, state, path)
  ...
  local bindings = normalize_bindings(node.bindings, path .. ".bindings", ctx)
  applet_expect(type(node.child) == "table", path .. ".child", "must be a layout node", 4)
  local next_state = {
    scopes = {},
    layer = state.layer,
    layer_panes = state.layer_panes,
    modal = state.modal,
    zindex = state.zindex,
    focusable = state.focusable,
  }
  for _, scope in ipairs(state.scopes) do
    next_state.scopes[#next_state.scopes + 1] = scope
  end
...
local function split_signature(node, items)
  ...
  for _, item in ipairs(items) do
    values[#values + 1] = item.signature
  end
...
local function project_split(ctx, node, rect, state, path)
  ...
  local key = register_key(ctx.node_keys, node.key, path .. ".key")
  applet_expect(
    node.axis == "vertical" or node.axis == "horizontal",
    path .. ".axis",
    "must be vertical or horizontal",
    4
  )
  applet_expect(
    node.revision == nil or type(node.revision) == "string" or finite(node.revision),
    path .. ".revision",
    "must be a finite number or string",
    4
  )
  applet_expect(
    type(node.children) == "table" and #node.children > 0,
    path .. ".children",
    "must be a non-empty list",
    4
  )
  applet_expect(getmetatable(node.children) == nil, path .. ".children", "must be a plain list", 4)
  local child_count = 0
  for _ in pairs(node.children) do
    child_count = child_count + 1
  end
  applet_expect(child_count == #node.children, path .. ".children", "must be a dense list", 4)
  ...
  if override and override.signature == signature and type(override.sizes) == "table" and #override.sizes == #items then
    local total = 0
    for index, size in ipairs(override.sizes) do
      local item = assert(items[index])
      if not integer(size) or size < item.min or (item.max and size > item.max) then
        total = -1
        break
      end
      total = total + size
  ...
  for index, item in ipairs(items) do
    ...
    local projected = layout_node(ctx, item.node, child_rect, state, path .. ".children." .. index .. ".child")
    children[#children + 1] = {
      key = item.key,
      size = item.size,
      min = item.min,
  ...
  local split_sizes = {}
  for _, item in ipairs(items) do
    split_sizes[#split_sizes + 1] = item.size
  end
  ctx.splits[key] = {
    key = key,
    axis = node.axis,
...
local function project_layer(ctx, node, index, path)
  ...
  local container
  if container_key == "editor" then
    container = ctx.editor
  elseif container_key == "applet" then
    container = ctx.bounds
  else
    nonempty(container_key, path .. ".container")
    local pane = ctx.panes[container_key]
    applet_expect(pane ~= nil, path .. ".container", "must name an already-declared Pane", 4)
    container = pane.outer
  end
  local anchor = node.anchor or "center"
  applet_expect(anchors[anchor], path .. ".anchor", "is not recognized", 4)
  local width = resolve_layer_dimension(node.width, container.width, node.child, "horizontal", ctx, path .. ".width")
  local height = resolve_layer_dimension(node.height, container.height, node.child, "vertical", ctx, path .. ".height")
  applet_expect(width > 0 and height > 0, path, "must resolve to a positive rectangle", 4)
  local rect = anchor_rect(container, width, height, anchor)
  local zindex = node.zindex or ((ctx.host.base_zindex or 40) + 30 + index * 10)
  applet_expect(integer(zindex) and zindex > 0, path .. ".zindex", "must be a positive integral value", 4)
  for _, field in ipairs({ "modal", "enter", "restore_focus" }) do
    applet_expect(node[field] == nil or type(node[field]) == "boolean", path .. "." .. field, "must be a boolean", 4)
  end
  local layer = {
    key = key,
  ...
  applet_expect(
    not layer.modal or not layer.enter or #layer.panes > 0,
    path,
    "modal entry requires a focusable Pane",
    4
  )
...
local function normalize_tree_focus(value, ctx)
  ...
  if value.initial ~= nil then
    nonempty(value.initial, "Applet layout.focus.initial")
  end
  local intent
  if value.intent ~= nil then
    ...
    applet_expect(
      intent.revision == nil or type(intent.revision) == "string" or finite(intent.revision),
      "Applet layout.focus.intent.revision",
      "must be a finite number or string",
      4
    )
  end
  local initial = value.initial or ctx.pane_order[1]
  applet_expect(initial == nil or ctx.panes[initial] ~= nil, "Applet layout.focus.initial", "must name a Pane", 4)
  applet_expect(intent == nil or ctx.panes[intent.key] ~= nil, "Applet layout.focus.intent.key", "must name a Pane", 4)
...
function M.compile(opts)
  applet_expect(type(opts) == "table", "Applet.layout.compile", "options must be a table", 3)
  local tree = opts.tree
  fields(tree, tree_fields, "Applet layout")
  applet_expect(type(tree.root) == "table", "Applet layout.root", "must be a frame node", 3)
  ...
  applet_expect(type(ctx.measurements) == "table", "Applet measurements", "must be a table", 3)
  applet_expect(type(ctx.overrides) == "table", "Applet overrides", "must be a table", 3)
  applet_expect(type(ctx.handlers) == "table", "Applet handlers", "must be a table", 3)
  applet_expect(
    ctx.has_action == nil or type(ctx.has_action) == "function",
    "Applet has_action",
    "must be a function",
    3
  )
  local root_bindings = normalize_bindings(tree.bindings, "Applet layout.bindings", ctx)
  ...
  applet_expect(frame.type == "frame", "Applet layout.root.type", "must be frame", 3)
  local frame_key = register_key(ctx.node_keys, frame.key, "Applet layout.root.key")
  applet_expect(type(frame.child) == "table", "Applet layout.root.child", "must be an Applet layout node", 3)
  local topology = layout_node(ctx, frame.child, bounds, {
    scopes = {},
    layer = nil,
    layer_panes = nil,
    modal = false,
    zindex = host.base_zindex,
    focusable = true,
  }, "Applet layout.root.child")
  local declared_layers = frame.layers or {}
  applet_expect(
    type(declared_layers) == "table" and getmetatable(declared_layers) == nil,
    "Applet layout.root.layers",
    "must be a plain list",
    3
  )
  for index, layer in ipairs(declared_layers) do
    applet_expect(
      type(layer) == "table" and layer.type == "layer",
      "Applet layout.root.layers." .. index,
      "must be a layer node",
      3
    )
    project_layer(ctx, layer, index, "Applet layout.root.layers." .. index)
  end
  local layer_count = 0
  for _ in pairs(declared_layers) do
    layer_count = layer_count + 1
  end
  applet_expect(layer_count == #declared_layers, "Applet layout.root.layers", "must be a dense list", 3)
  ...
  if modal_index then
    for index = modal_index, #ctx.layers do
      for _, key in ipairs(ctx.layers[index].panes) do
        modal_boundary[#modal_boundary + 1] = key
      end
    end
  end
  if #modal_boundary > 0 then
    local allowed = {}
    for _, key in ipairs(modal_boundary) do
      allowed[key] = true
    end
  ...
  if #modal_boundary > 0 then
    ...
    if focus.intent then
      applet_expect(
        allowed[focus.intent.key],
        "Applet layout.focus.intent.key",
        "must target the active modal boundary",
        3
      )
    end
