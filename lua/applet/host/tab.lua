...
local function projected_topology(topology, records)
  if topology.type == "pane" then
    ...
    if not record or record.suppressed or record.descriptor.projection.kind ~= "split" then
      return nil
    end
  ...
  if #children == 1 then
    return assert(children[1]).child
  end
...
function Driver:foreign_windows()
  if not base.valid_tab(self.tab) then
    return 0
  end
  local count = 0
  for _, window in ipairs(vim.api.nvim_tabpage_list_wins(self.tab)) do
    if not self.applet._windows[window] then
      count = count + 1
    end
  end
  return count
...
function Driver:_create_tab(frame, records)
  ...
  pcall(vim.api.nvim_tabpage_set_var, tab, "applet_label", frame.host.label)
  if base.valid_tab(origin_tab) then
    vim.api.nvim_set_current_tabpage(origin_tab)
  end
  base.with_tab(tab, function()
    local topology = assert(projected_topology(frame.topology, records), "tab Host requires one mounted main Pane")
...
function Driver:_apply_sizes(topology)
  ...
  for _, child in ipairs(topology.children) do
    ...
    if record and base.valid_window(record.window) then
      ...
      self.applet.counters.split_size_changes = self.applet.counters.split_size_changes + 1
    end
...
function Driver:_open_layers(frame, records)
  for _, key in ipairs(frame.pane_order) do
    ...
    if record and record.descriptor.projection.kind == "floating" and not record.suppressed then
      local config = layer_config(record)
      if not self.published or self.staging then
        config.hide = true
      end
...
function Driver:publish(_, records)
  ...
  if shadow then
    ...
    if base.valid_tab(shadow.old_tab) then
      base.close_tab(shadow.old_tab, true)
    end
...
function Driver:_close_owned_windows(records, floats_only)
  for _, record in pairs(records) do
    if record.window and (not floats_only or record.descriptor.projection.kind == "floating") then
...
function Driver:_rebuild(frame, records)
  ...
  local root = vim.api.nvim_get_current_win()
  local temporary = vim.api.nvim_win_get_buf(root)
  pcall(vim.api.nvim_tabpage_set_var, staging_tab, "applet_label", frame.host.label)
  ...
  if not built then
    ...
    for key, window in pairs(old_windows) do
      ...
      if base.valid_window(window) then
        self.applet._windows[window] = key
      end
    end
    if base.valid_tab(previous_tab) then
...
function Driver:rollback(records)
  ...
  if transaction then
    local retained = {}
    for _, saved in pairs(transaction.windows) do
      retained[saved.window] = true
    end
...
function Driver:reconcile(previous, frame, records)
  ...
  assert(self:is_open(), "tab Host is closed")
  pcall(vim.api.nvim_tabpage_set_var, self.tab, "applet_label", frame.host.label)
  ...
  for _, key in ipairs(frame.pane_order) do
    ...
    if
      record
      and record.descriptor.projection.kind == "split"
      and not record.suppressed
      and not base.window_displays(record.window, record.buffer)
    then
      missing_main = true
      break
    end
  ...
  base.with_tab(self.tab, function()
    ...
    for key, record in pairs(records) do
      if record.window and not desired[key] then
        local saved = assert(
          self.transaction and self.transaction.windows[key],
          "removed tab Pane must belong to the active transaction"
        )
        assert(
          saved.window == record.window and record.descriptor.projection.kind == "floating",
    ...
    if previous then
      for _, split_key in ipairs(frame.split_order) do
        ...
        if not before or before.signature ~= after.signature then
          sizing_changed = true
          break
        end
      end
    end
    if sizing_changed then
      self:_apply_sizes(topology)
    end
  end)
  return true
...
function Driver:focus(record)
  assert(self:is_open() and record and base.valid_window(record.window), "tab Host focus requires an open mounted Pane")
...
function Driver:release(records)
  ...
  if self.pending_shadow or self.transaction then
  ...
  if base.valid_tab(tab) then
    ...
    if foreign == 0 then
      ...
      if #tabs == 1 then
        vim.cmd("tabnew")
      elseif vim.api.nvim_get_current_tabpage() == tab and self.origin and base.valid_tab(self.origin.tab) then
