...
local function model_entry(value)
  if type(value) ~= "table" or util.is_list(value) or not model_config.safe_id(value.slug) then
    return nil
  end
  ---@type Neoagent.CodexCatalogModel
  local result = { id = value.slug, reasoning_levels = {} }
  if safe_text(value.display_name, 256) then
  ...
  if type(value.input_modalities) == "table" and util.is_list(value.input_modalities) then
    local input = {}
    for _, modality in ipairs(value.input_modalities) do
      if modality == "text" or modality == "image" then
        if not vim.tbl_contains(input, modality) then
  ...
  if context_window ~= nil then
    if not positive_integer(context_window) then
      return nil
    end
    result.context_window = context_window
  end
  if value.use_responses_lite ~= nil then
    if type(value.use_responses_lite) ~= "boolean" then
      return nil
    end
  ...
  if type(reasoning) ~= "table" or not util.is_list(reasoning) then
    return nil
  end
  local seen = {}
  ...
  if tiers ~= nil then
    if type(tiers) ~= "table" or not util.is_list(tiers) then
      return nil
    end
    result.service_tiers = {}
    for _, tier in ipairs(tiers) do
      local id = type(tier) == "table" and tier.id or tier
      if not safe_text(id, 128) then
        return nil
      end
...
local function parse(value)
  if
    ...
    or type(value.models) ~= "table"
    or not util.is_list(value.models)
    or #value.models > 256
  then
    return nil
  end
  ...
  for _, raw in ipairs(value.models) do
    ...
    if not model or seen[model.id] then
      return nil
    end
    seen[model.id] = true
  ...
  return async.run(
    ...
    function()
      ...
      if resolved.configured ~= true or resolved.credential_type ~= "oauth" then
        error(util.error("auth", "ChatGPT subscription login is required to load Codex models"), 0)
      end
      local auth_headers = type(resolved.request_opts) == "table" and resolved.request_opts.headers or {}
      ...
      local fetched = transport
        .fetch({
          request = {
            url = base_url(assert(provider.base_url, "Codex catalog requires base_url"))
      ...
      if type(fetched.status) ~= "number" or fetched.status < 200 or fetched.status >= 300 then
        ---@type Neoagent.ProviderHttpError
        local err = util.error(
          "provider",
          "Codex model catalog request failed" .. (fetched.status and " (HTTP " .. fetched.status .. ")" or "")
        )
      ...
      if not models then
        error(util.error("provider", "Codex returned an invalid model catalog"), 0)
      end
      return {
        ok = true,
        models = models,
