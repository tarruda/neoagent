...
local function iso_timestamp(value)
  ...
  local year_text, month_text, day_text, hour_text, minute_text, second_text =
  ...
  local timestamp = math.floor(days * 86400 + hour * 3600 + minute * 60 + math.min(second, 59))
  ...
  if
    ...
    or roundtrip.sec ~= math.min(second, 59)
  then
    return nil
  end
  return timestamp
...
local function response_error(status, resource)
  ...
  return "OpenCode Go " .. resource .. " request failed (HTTP " .. tostring(status) .. ")"
end
...
local function parse_usage(value)
  if type(value) ~= "table" or util.is_list(value) or type(value.usage) ~= "table" or util.is_list(value.usage) then
    return nil
  end
  ...
  for _, id in ipairs({ "rolling", "weekly", "monthly" }) do
    ...
    local resets_at = type(source) == "table" and iso_timestamp(source.resetsAt) or nil
    if
      not finite(percent)
      or percent < 0
      or percent > 100
...
local function parse_models(value)
  ...
  for _, source in ipairs(value.data) do
    ...
    if not safe_id(id) or seen[id] then
      return nil
    end
    seen[id] = true
    result[#result + 1] = id
...
function M.new(opts)
  opts = opts or {}
  assert(type(opts.base_url) == "string" and opts.base_url ~= "", "OpenCode Go base_url is required")
  ...
  assert(type(timeout_ms) == "number" and timeout_ms > 0, "OpenCode Go timeout_ms must be positive")
  local base_url = opts.base_url:gsub("/+$", "")
  local ambient_api_key = opts.ambient_api_key or function()
    return vim.env.OPENCODE_API_KEY
  end
  assert(type(ambient_api_key) == "function", "OpenCode Go ambient_api_key must be a function")
  ...
  local function request(path, resource, headers)
    return async.run(
      ...
      function()
        ...
        if not status then
          error(util.error("provider", "OpenCode Go " .. resource .. " response has no HTTP status"), 0)
        end
        ...
        if type(decoded) ~= "table" then
          error(util.error("provider", "OpenCode Go " .. resource .. " response contains invalid JSON"), 0)
        end
  ...
  function client:usage(ctx)
    assert(type(ctx) == "table" and type(ctx.resolve_auth) == "function", "OpenCode Go usage requires auth resolution")
    return async.run(
      ...
      function()
        ...
        if resolved.configured then
          key = bearer_from(type(resolved.request_opts) == "table" and resolved.request_opts.headers or nil)
        else
          local ok, value = pcall(ambient_api_key)
          if not ok then
            error(util.error("provider", "Failed to resolve OPENCODE_API_KEY", value), 0)
          end
        ...
        if not key then
          error(util.error("provider", "Connect OpenCode Go or set OPENCODE_API_KEY to load usage"), 0)
        end
        ...
        if not usage then
          error(util.error("provider", "OpenCode Go returned invalid usage data"), 0)
        end
  ...
  function client:models()
    return async.run(
      ...
      function()
        ...
        if not models then
          error(util.error("provider", "OpenCode Go returned an invalid model catalog"), 0)
        end
