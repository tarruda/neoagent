...
local function add_usage(left, right)
  ...
  if right and right.cost then
    result.cost = add_cost(result.cost, right.cost)
  end
  return result
end
...
local function combine(first, result)
  ...
  for _, block in ipairs(result.message and result.message.content or {}) do
    ...
    if copied.type == "text" or copied.type == "thinking" then
      copied.index = 1
    end
    ...
    return async.run(
      ...
      function(run)
        ...
        if
          first.ok
          or not first.error
          or not missing_tool_call(first.error)
          or not first.message
          or run:is_cancelled()
        then
          return first
        end
        ...
        run:emit({
          type = "warning",
          message = "Provider bug: OpenCode Go's qwen3.8-flash "
        ...
        if not ok then
          result = { ok = false, error = util.normalize_error(result, "model") }
        end
        return combine(first.message, result)
      end,
      { on_event = opts.on_event, on_done = opts.on_done, error_kind = "model" }
    )
  end
