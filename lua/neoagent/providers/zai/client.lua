...
local function parse_models(value)
  ...
  for _, entry in ipairs(value.data) do
    ...
    if not safe_text(id, 512) or seen[id] then
      return nil
    end
    seen[id] = true
    result[#result + 1] = id
...
local function parse_balance(value)
  ...
  if type(data) ~= "table" or util.is_list(data) then
    return nil
  end
  local available = amount(data.available_balance)
  local total = amount(data.total_balance) or available
  local currency = data.currency
  if available == nil or total == nil or currency ~= nil and not safe_text(currency, 16) then
    return nil
  end
  return {
    total = total,
...
local function window(source, kind)
  ...
  if not finite(unit) or unit % 1 ~= 0 or not finite(count) or count <= 0 or count % 1 ~= 0 or count > 10000 then
    return nil
  end
  if unit == 3 then
  ...
  if unit == 4 then
    return count == 1 and "Daily" or (tostring(count) .. "-day"), count * 24 * 60
  end
  if unit == 5 then
    return count == 1 and "Monthly" or (tostring(count) .. "-month"), count * 30 * 24 * 60
  end
  if unit == 6 then
...
local function parse_limit(source)
  if type(source) ~= "table" or util.is_list(source) then
    return false
  end
  local kind = source.type
  if kind ~= "TOKENS_LIMIT" and kind ~= "CREDIT_LIMIT" and kind ~= "TIME_LIMIT" then
    return nil
  end
  local percentage = source.percentage
  if percentage ~= nil and (not finite(percentage) or percentage < 0 or percentage > 100) then
    return false
  end
  local current = source.currentValue ~= nil and amount(source.currentValue) or nil
  local maximum = source.usage ~= nil and amount(source.usage) or nil
  local remaining_count = source.remaining ~= nil and amount(source.remaining) or nil
  if
    source.currentValue ~= nil and current == nil
    or source.usage ~= nil and maximum == nil
    or source.remaining ~= nil and remaining_count == nil
  then
    return false
  end
  if current ~= nil or maximum ~= nil or remaining_count ~= nil then
    if
      ...
      or remaining_count ~= nil and remaining_count > maximum
    then
      return false
    end
    if current == nil then
...
local function parse_quota(value)
  local data = type(value) == "table" and not util.is_list(value) and value.data or nil
  if
    type(data) ~= "table"
    or util.is_list(data)
    or type(data.limits) ~= "table"
    or not util.is_list(data.limits)
    or #data.limits == 0
    or #data.limits > 32
  then
    return nil
  ...
  if plan ~= nil and not safe_text(plan, 128) then
    return nil
  end
  ...
  table.sort(ordered, function(left, right)
    ...
    if left_time ~= right_time then
      return not left_time
    end
    return left.order < right.order
  end)
  local limits = {}
  for _, entry in ipairs(ordered) do
    limits[#limits + 1] = entry.limit
  end
...
local function authorization(headers, bearer)
  ...
  if key == "" then
    return nil, util.error("provider", "Z.AI credentials returned no API key header")
  end
...
function M.new(opts)
  opts = opts or {}
  assert(type(opts.management_url) == "string" and opts.management_url ~= "", "Z.AI management_url is required")
  ...
  function client:models(ctx)
    return async.run(
      ...
      function()
        ...
        if not models then
          error(util.error("provider", "Z.AI returned an invalid model catalog"), 0)
        end
  ...
  function client:balance(ctx)
    return async.run(
      ...
      function()
        ...
        local fetched = request:get("/api/paas/v4/balance", "balance", headers):await()
        if fetched.ok == false then
          error(fetched.error, 0)
        end
  ...
  function client:quota(ctx)
    return async.run(
      ...
      function()
        ...
        local fetched = request:get("/api/monitor/usage/quota/limit", "plan quota", headers):await()
        if fetched.ok == false then
          error(fetched.error, 0)
        end
