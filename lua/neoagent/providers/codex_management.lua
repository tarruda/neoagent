...
function M.new(opts)
  opts = opts or {}
  assert(type(opts.base_url) == "string" and opts.base_url ~= "", "Codex management base_url is required")
  ...
  assert(type(timeout_ms) == "number" and timeout_ms > 0, "Codex management timeout_ms must be positive")
    ...
    return async.run(
      ...
      function()
        ...
        if resolved.configured ~= true then
          error(util.error("provider", "Sign in with ChatGPT to load Codex account information"), 0)
        end
          ...
          error(
            util.error(
              ...
              "ChatGPT subscription login is required; API key authentication "
                .. "cannot load Codex account information"
            ),
            0
          )
        end
        local headers = util.copy(type(resolved.request_opts) == "table" and resolved.request_opts.headers or {})
        if body ~= nil then
        ...
        if not status then
          error(util.error("provider", "Codex account response has no HTTP status"), 0)
        end
        ...
        if type(decoded) ~= "table" or util.is_list(decoded) then
          error(util.error("provider", "Codex account response contains invalid JSON"), 0)
        end
        return {
          ok = true,
          value = decoded,
  ...
  function client:redeem(ctx, redeem_request_id, credit_id)
    assert(type(redeem_request_id) == "string" and redeem_request_id ~= "", "redeem_request_id is required")
    local body = { redeem_request_id = redeem_request_id }
    if credit_id ~= nil then
      assert(type(credit_id) == "string" and credit_id ~= "", "credit_id must be a non-empty string")
      body.credit_id = credit_id
    end
    return request(ctx, "/wham/rate-limit-reset-credits/consume", "POST", body)
  end
