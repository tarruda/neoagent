...
local DEFAULT_GATEWAY_URL = "https://bailian-singapore-cs.alibabacloud.com"
...
local USAGE_API = "zeldaHttp.apikeyMgr./tokenplan/personal/api/v2/usage"
...
local function encode_fields(fields)
  ...
  for _, name in ipairs(names) do
    result[#result + 1] = vim.uri_encode(name, "rfc2396") .. "=" .. vim.uri_encode(tostring(fields[name]), "rfc2396")
  end
...
local function window(value, percentage_name, reset_name)
  ...
  if percentage == nil and reset == nil then
    return nil
  end
  if percentage ~= nil and (not finite(percentage) or percentage < 0 or percentage > 1) then
    return false
  end
  local resets_at = reset ~= nil and timestamp(reset) or nil
  if reset ~= nil and not resets_at then
    return false
  end
  local result = {}
  if percentage ~= nil then
    result.used = percentage
  end
  if resets_at then
...
local function unwrap(value)
  ...
  if data_v2 then
    ...
    return inner and type(inner.data) == "table" and inner.data or inner or data_v2
  end
...
local function parse_usage(value)
  ...
  local five_hour = window(value, "per5HourPercentage", "per5HourResetTime")
  local seven_day = window(value, "per1WeekPercentage", "per1WeekResetTime")
  if five_hour == false or seven_day == false then
    return nil
  end
  local result = {}
  if five_hour then
...
local function authorization(resolved)
  if type(resolved) ~= "table" or resolved.ok == false then
    error(type(resolved) == "table" and resolved.error or util.error("auth", "Alibaba Cloud authorization failed"), 0)
  end
  if not resolved.configured then
    error(
      util.error("auth", "Log in to the Alibaba Cloud dashboard through the Provider Shell " .. "to load quotas"),
      0
    )
  end
  local headers = type(resolved.request_opts) == "table" and resolved.request_opts.headers or nil
  for name, value in pairs(headers or {}) do
    if
      type(name) == "string"
      and name:lower() == "authorization"
      and type(value) == "string"
      and value:match("^[Bb]earer%s+%S")
    then
      return value
    end
  end
  error(util.error("auth", "Alibaba Cloud dashboard authorization returned no bearer token"), 0)
end
...
function M.new(opts)
  ...
  assert(gateway_url:match("^https?://[^/]+"), "Alibaba Token Plan gateway_url must be an HTTP origin")
  ...
  function client:usage(ctx)
    assert(
      type(ctx) == "table" and type(ctx.resolve_auth) == "function",
    ...
    return async.run(
      ...
      function()
        ...
        if not status then
          error(util.error("provider", "Alibaba Cloud quota response has no HTTP status"), 0)
        end
        if status < 200 or status >= 300 then
          ---@type Neoagent.ProviderHttpError
          local err = util.error("provider", "Alibaba Cloud quota request failed (HTTP " .. tostring(status) .. ")")
          err.status = status
          error(err, 0)
        end
        local value = fetched.body
        if type(value) ~= "table" then
          error(util.error("provider", "Alibaba Cloud quota response contains invalid JSON"), 0)
        end
        local envelope = type(value.data) == "table" and value.data or nil
        if envelope and envelope.success == false and envelope.errorCode ~= nil then
          local code = tostring(envelope.errorCode)
          if code:find("NotLogined", 1, true) then
            error(util.error("auth", "Alibaba Cloud dashboard authorization expired"), 0)
          end
          error(util.error("provider", "Alibaba Cloud console gateway rejected the quota request"), 0)
        end
        local usage = parse_usage(value)
        if not usage then
          error(util.error("provider", "Alibaba Cloud returned invalid usage data"), 0)
        end
