...
function M.resolve(ctx, opts)
  ...
  assert(type(opts.ambient_api_key) == "function", "ambient_api_key must be a function")
  return async.run(
    ...
    function()
      ...
      if resolved.configured then
        ...
        if type(headers) ~= "table" or util.is_list(headers) then
          error(util.error("auth", (opts.name or "Provider") .. " credentials returned no request headers"), 0)
        end
        return { ok = true, headers = util.copy(headers) }
      end
      local ok, key = pcall(opts.ambient_api_key)
      if not ok then
        error(util.error("auth", "Failed to resolve " .. tostring(opts.environment or "provider API key"), key), 0)
      end
      if type(key) ~= "string" or util.trim(key) == "" then
        error(util.error("auth", opts.missing_message or ((opts.name or "Provider") .. " credentials are required")), 0)
      end
      key = util.trim(key)
      local headers
