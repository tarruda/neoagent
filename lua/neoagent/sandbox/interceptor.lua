...
---@type table<Neoagent.DeferredSandboxLease, boolean>
local deferred_leases = {}
...
local function denied_result(value, platform, evidence)
  ...
  if evidence and denial_text(evidence.denial_output) then
    return true
  end
...
local function resolve_profile(self, ctx)
  ...
  if self._fingerprint then
    profile = util.copy(assert(self._configured_profile))
  else
...
---@param deferred Neoagent.DeferredSandboxLease
local function dispose_deferred(deferred)
  if deferred.disposed then
    return
  end
  deferred.disposed = true
  pcall(deferred.lease.dispose, deferred.lease, deferred.reason)
end

---@param deferred Neoagent.DeferredSandboxLease
local function retain_worker_wait(deferred)
  deferred_leases[deferred] = true
  local function release()
    close_timer(deferred.timer)
...
---@param worker Neoagent.WorkerLease
---@param reason string
...
---@param lease Neoagent.WorkerLease
---@param reason string
local function cancel_and_reap(connection, lease, reason)
  local cancelled = pcall(connection.cancel, connection)
  if not cancelled then
    connection:abort()
  end
  retain_cancelled_lease(connection, lease, reason)
end
...
local function wait_for_worker(lease, reason)
  local timer = assert(vim.uv.new_timer())
  ...
  timer:start(WORKER_EXIT_GRACE_MS, 0, function()
    close_timer(timer)
    dispose_deferred(deferred)
  end)
  local waited, value = pcall(lease.wait, lease)
  if not waited then
...
local function sandbox_error(err, profile, platform, ran_restricted)
  ...
  if err.kind == "sandbox_denied"
      or err.kind == "tool" and (denial_text(err.message) or denial_text(bounded(err.detail))) then
    fields.denied = true
  ...
  if err.kind == "sandbox_unavailable" or err.kind == "worker_start" or err.kind == "protocol"
      or err.kind == "sandbox" then
...
local function open_worker(self, profile, call)
  ...
  if not admitted then
    ...
    if err.kind == "cancelled" then
      cancel_and_reap(connection, lease, "restricted Tool worker admission cancelled")
      error(err, 0)
    end
    connection:abort()
    dispose_and_reap(lease, "restricted Tool worker failed platform admission")
    error(err, 0)
  end
  ...
  if not opened then
    ...
    if err.kind == "cancelled" then
      cancel_and_reap(connection, lease, "restricted Tool worker opening cancelled")
      error(err, 0)
    end
    connection:abort()
    dispose_and_reap(lease, "restricted Tool worker failed to open")
    error(err, 0)
  end
...
function Interceptor:wrap(next_execute)
  ...
  return function(tool, arguments, ctx)
    ...
    local function invoke(method, request_value, operation_call)
      intercepted = true
      policy:authorize(method, request_value, operation_call)
      if not connection then
        connection, lease = open_worker(self, profile, call)
      end
      ran_restricted = true
      local completed, value = pcall(registry.invoke, connection, method, request_value, operation_call, function(value)
        policy_evidence = value
      end)
      if not completed then
        local err = util.normalize_error(value, "tool")
        -- Once the request is handed to RPC, a broken channel or invalid
        -- semantic result cannot establish whether the operation took effect.
        -- Keep this Tool-specific retry classification outside RpcConnection.
        if err.kind == "protocol" or err.kind == "artifact" then
    ...
    connection = assert(connection)
    if execution_err and execution_err.kind == "cancelled" then
      cancel_and_reap(connection, lease, "restricted Tool invocation cancellation did not settle")
      error(execution_err, 0)
    end
    ...
    if execution_err then
      return sandbox_error(execution_err, profile, self._platform.name, ran_restricted)
    end
    ---@cast value Neoagent.ToolResult
    local cleanup_error = not closed and util.normalize_error(close_err, "protocol") or nil
    if closed and not waited then
      cleanup_error = util.normalize_error(worker_result, "sandbox_unavailable")
    elseif closed and worker_result and worker_result.error then
      cleanup_error = util.normalize_error(worker_result.error, "sandbox_unavailable")
    elseif closed and worker_result and worker_result.code ~= 0 then
      cleanup_error = util.error(
        "protocol",
        "Tool worker failed during shutdown",
        worker_result.stderr ~= "" and worker_result.stderr or nil
      )
    end
    local operation_denied = denied_result(value, self._platform.name, policy_evidence)
    if cleanup_error then
      value = result.cleanup_failure(value, cleanup_error.message, {
        backend = self._platform.name,
        profile = profile.id,
...
function M.new(opts)
  ...
  local configured, fingerprint
  if type(opts.profile) == "table" then
    configured, fingerprint = profile_module.validate(opts.profile, { paths = paths })
  end
...
M.worker_environment = worker_environment
return M
