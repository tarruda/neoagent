...
---@class Neoagent.EditFileDependencies
...
---@return Neoagent.EditFileDependencies
local function dependencies(options)
  ...
  if fingerprint == nil then
    fingerprint = require("neoagent.fs").content_fingerprint
  end
  assert(type(fs) == "table", "Tool filesystem is required")
  assert(type(fingerprint) == "function", "Tool content fingerprint is required")
  return {
    fs = fs,
...
local function validate_request(value)
  ...
  ---@cast value table
  common.fields(value, { path = true, resolved_path = true, edits = true }, "edit_file request")
  assert(
    type(value.edits) == "table" and util.is_list(value.edits) and #value.edits > 0,
    "edit_file edits must be a non-empty list"
  )
  ...
  return common.request({
    ...
    resolved_path = common.absolute_path(value.resolved_path, "edit_file resolved path"),
    edits = edits,
  }, "edit_file request")
end
...
local function diff_details(path, resolved_path, old, new)
...
---@param deps Neoagent.EditFileDependencies
...
local function run(request, _call, deps)
  ...
  local details = diff_details(request.path, absolute, content, changed)
  ...
  if not ok then
    common.filesystem_error("Could not edit file " .. request.path, replace_err)
  end
  return {
    content = {
      {
