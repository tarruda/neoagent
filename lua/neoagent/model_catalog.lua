...
local function finite_timestamp(value)
  return type(value) == "number" and value >= 0 and value % 1 == 0 and value == value and value ~= math.huge
end
...
local function validator(value)
  ...
  for key in pairs(value) do
    if not VALIDATOR_FIELDS[key] then
      return nil
    end
  end
  local result = {}
...
local function cache_record(provider_id, value, expected_fingerprint)
  ...
  if
    type(value.source_fingerprint) ~= "string"
    or not value.source_fingerprint:match("^[0-9a-f]+$")
    or #value.source_fingerprint ~= 64
  then
    return nil
  end
  ...
  if not finite_timestamp(timestamp) then
    return nil
  end
  ...
  if value.validator ~= nil and not selected_validator then
    return nil
  end
  return {
    models = models,
...
local function callback_error(provider_id, model_id, value)
  return util.error("model", "Model transform failed for " .. provider_id .. "/" .. model_id, value)
end
...
local function model_configuration(values, label)
  ...
  assert(
    values == nil or type(values) == "table" and (next(values) == nil or not util.is_list(values)),
    "ModelCatalog " .. label .. " must be a keyed table"
  )
  for model_id, model in pairs(values or {}) do
    assert(model_config.safe_id(model_id), "ModelCatalog " .. label .. " ids must be safe non-empty text")
    assert(
      model == false or type(model) == "table" and (next(model) == nil or not util.is_list(model)),
...
local function fallback_source(catalog)
  if #catalog._seed > 0 then
    return "packaged"
  end
  if #catalog._configured_inventory > 0 then
    return "configured"
  end
  return "empty"
end
...
function Catalog:_build(discoveries)
  local normalized, normalize_err = model_config.normalize_discoveries(self._provider_id, discoveries)
  ...
  for _, model_id in ipairs(ids) do
    ...
    if self._transform then
      ...
      elseif type(transformed) ~= "table" or util.is_list(transformed) then
        return nil, callback_error(self._provider_id, model_id, "callback must return a model table or false")
      else
    ...
    if type(model) == "table" and not self._removals[model_id] then
      ...
      local valid, valid_err = model_config.validate(self._provider_id, model_id, combined)
      if not valid then
...
function Catalog:_report_diagnostic(message, err, level)
  ...
  pcall(self._report, "neoagent: " .. message .. ": " .. selected.message, level or vim.log.levels.WARN)
  return selected
end
...
function Catalog:_notify()
  ...
  for listener in pairs(self._listeners) do
    ...
    if not ok then
      self:_report_diagnostic("model catalog subscriber failed for " .. self._provider_id, err, vim.log.levels.ERROR)
    end
  end
end
...
function Catalog:_publish(discoveries, opts)
  ...
  if next(models) == nil and opts.allow_empty ~= true then
    return nil, util.error("provider", "Model catalog discovery returned an empty effective inventory")
  end
  self._discoveries = normalized
  self._models = models
  self._validated_at = opts.validated_at or self._validated_at
  if opts.validator_set then
    self._validator = util.copy(opts.validator)
  end
...
function Catalog:_persist()
  if not self._store or self._definition.source_id == nil then
    return nil
  end
  if not self._source_fingerprint then
    return util.copy(self._persistence_error or util.error("provider", "Model catalog source identity is unavailable"))
  end
  local called, saved, err = pcall(self._store.write, self._store, self._provider_id, self:_cache_value())
  ...
  return self:_report_diagnostic("failed to persist " .. self._provider_id .. " model catalog", failure)
end
...
function Catalog:_refresh_source_fingerprint()
  ...
  self._persistence_error = self._store and self._definition.source_id and not fingerprint and util.copy(err) or nil
...
function Catalog:_auth_changed(event)
  ...
  if event.kind == "refresh" and current == previous then
    return
  end
  self._generation = self._generation + 1
  self:_cancel_timer()
  if self._active and not self._active:is_done() then
    self._active:cancel()
  end
  self._active = nil
  self:_reset_inventory("failed to reset model catalog after authentication change")
...
function Catalog:_schedule(milliseconds, retry)
  self:_cancel_timer()
  if self._destroyed or not self._started or not self._discover then
    return
  end
  ...
  if
    ...
    or type(timer.is_closing) ~= "function"
  then
    self:_diagnose(
      "failed to create model catalog timer",
  ...
  if not started then
    self._timer = nil
    close_timer(timer)
    self:_diagnose("failed to start model catalog timer", start_err, vim.log.levels.ERROR)
  end
end
...
function Catalog:_schedule_retry(err)
  ...
  local delay = math.min(RETRY_MAX_MS, RETRY_BASE_MS * 2 ^ (self._retry_attempt - 1))
...
function Catalog:_resolve_auth()
  ...
  return self._auth:resolve(self._provider.auth, {
    optional = self._provider.auth_optional == true or self._provider.api_key ~= nil,
  })
end
...
function Catalog:snapshot()
  local stale = self._discover ~= nil
      and (self._last_error ~= nil or self._validated_at == nil or self._now() - self._validated_at > self._ttl_ms)
    or false
  return {
    ...
    source = self._source,
    refresh = {
      state = self._active and not self._active:is_done() and "refreshing" or self._last_error and "failed" or "idle",
...
function Catalog:subscribe(listener)
  assert(type(listener) == "function", "model catalog subscriber must be a function")
  if self._destroyed then
    return function()
      return false
    end
...
function Catalog:start()
  ...
  local fresh = self._validated_at ~= nil and self._now() - self._validated_at <= self._ttl_ms
  if fresh then
...
function Catalog:refresh(opts)
  ...
  if not acquired or type(lease) ~= "table" or type(lease.release) ~= "function" then
  ...
  if
    generation_fingerprint ~= previous_fingerprint
    and (generation_fingerprint ~= nil or previous_fingerprint ~= nil)
  then
  ...
  run = async.run(
    ...
    function()
      ...
      if
        type(source_run) ~= "table"
        or type(source_run.await) ~= "function"
        or type(source_run.cancel) ~= "function"
      then
        error(util.error("provider", "Model catalog discover callback must return a Run"), 0)
      end
      local discovered = source_run:await()
      if type(discovered) == "table" and discovered.ok == false then
        error(discovered.error or util.error("provider", "Model catalog discovery failed"), 0)
      end
      if type(discovered) ~= "table" or discovered.ok ~= true then
        error(util.error("provider", "Model catalog discover callback returned an invalid result"), 0)
      end
      if discovered.unchanged ~= nil and type(discovered.unchanged) ~= "boolean" then
        error(util.error("provider", "Model catalog discover callback returned an invalid result"), 0)
      end
      if discovered.unchanged == true and discovered.models ~= nil then
        error(util.error("provider", "Unchanged model catalog discovery must not return models"), 0)
      end
      ...
      if discovered.unchanged == true then
        if #self._discoveries == 0 then
          error(util.error("provider", "Model catalog cannot be unchanged without prior discoveries"), 0)
        end
      ...
      local latest_fingerprint, fingerprint_err = self:_refresh_source_fingerprint()
      if latest_fingerprint ~= generation_fingerprint then
        self:_reset_inventory()
        error(fingerprint_err or util.error("provider", "Model catalog source changed during discovery"), 0)
      end
      ...
      if discovered.validator ~= nil and not next_validator then
        error(util.error("provider", "Model catalog returned an invalid validator"), 0)
      end
    ...
    end,
    {
      ...
      report = function(diagnostic)
        self._report(
          "neoagent callback failed during " .. diagnostic.phase .. ": " .. diagnostic.message,
          vim.log.levels.ERROR
        )
      end,
      on_done = function(result)
        ...
        if not released or value ~= true then
          self:_report_diagnostic(
            "failed to release model catalog use",
            released and release_err or value,
            vim.log.levels.ERROR
          )
        end
        if generation == self._generation then
          ...
          if not result.ok and result.error.kind ~= "cancelled" and not self._destroyed then
            if result.error.kind == "auth" then
              self:_record_error(result.error)
            else
              self:_diagnose("model catalog refresh failed for " .. self._provider_id, result.error)
            end
            self._revision = self._revision + 1
            self:_notify()
...
function Catalog:destroy()
  ...
  self._active = nil
  self._listeners = {}
  if self._auth_unsubscribe then
    pcall(self._auth_unsubscribe)
  end
...
function M.new(opts)
  opts = opts or {}
  assert(
    model_config.safe_provider_id(opts.provider_id),
    "ModelCatalog provider_id must be safe text without path separators"
  )
  local definition = opts.definition or {}
  assert(
    type(definition) == "table" and (next(definition) == nil or not util.is_list(definition)),
    "ModelCatalog definition must be an object"
  )
  for name in pairs(definition) do
    assert(DEFINITION_FIELDS[name], "unsupported ModelCatalog definition field " .. tostring(name))
  end
  local ttl_ms = definition.ttl_ms or DEFAULT_TTL_MS
  assert(
    type(ttl_ms) == "number" and ttl_ms > 0 and ttl_ms % 1 == 0 and ttl_ms < math.huge,
    "ModelCatalog ttl_ms must be a positive integer"
  )
  assert(
    definition.discover == nil or type(definition.discover) == "function",
    "ModelCatalog discover must be a function"
  )
  assert(
    definition.source_id == nil
      or type(definition.source_id) == "string"
        and definition.source_id ~= ""
        and #definition.source_id <= 256
  ...
  assert(
    definition.source_revision == nil
  ...
  assert(
    (definition.source_id == nil) == (definition.source_revision == nil),
    "ModelCatalog source_id and source_revision must be supplied together"
  )
  assert(
    definition.account_scoped == nil or type(definition.account_scoped) == "boolean",
    "ModelCatalog account_scoped must be a boolean"
  )
  assert(
    definition.account_scoped ~= true or definition.source_id ~= nil,
    "ModelCatalog account_scoped requires source identity"
  )
  assert(
    not (opts.store and definition.discover) or definition.source_id ~= nil,
    "persisted ModelCatalog discovery requires source identity"
  )
  assert(
    definition.transform_model == nil or type(definition.transform_model) == "function",
    "ModelCatalog transform_model must be a function"
  )
  assert(
    definition.source_options == nil or type(definition.source_options) == "function",
    "ModelCatalog source_options must be a function"
  )
  local provider = opts.provider or {}
  assert(
    type(provider) == "table" and (next(provider) == nil or not util.is_list(provider)),
    "ModelCatalog provider must be an object"
  )
  assert(
    not (definition.discover and next(provider.service_opts or {}) ~= nil)
      or type(definition.source_options) == "function",
    "ModelCatalog discovery with service_opts requires source_options"
  )
  assert(
    opts.store == nil
      or type(opts.store) == "table" and type(opts.store.read) == "function" and type(opts.store.write) == "function",
    "ModelCatalog store must provide read and write"
  )
  assert(opts.report == nil or type(opts.report) == "function", "ModelCatalog report must be a function")
  local overrides, removals = model_configuration(opts.models, "models")
  local additions = model_configuration(definition.additions, "catalog additions")
  local inventory = configured_inventory(additions, overrides, definition.discover == nil)
  ...
  assert(
    type(self._auth) == "table" and type(self._auth.resolve) == "function",
    "ModelCatalog authentication manager is invalid"
  )
  assert(
    self._credentials == nil
      or type(self._credentials) == "table" and type(self._credentials.ambient_api_key) == "function",
    "ModelCatalog ProviderCredentials value is invalid"
  )
  assert(type(self._now) == "function", "ModelCatalog now must be a function")
  assert(type(self._new_timer) == "function", "ModelCatalog new_timer must be a function")
  assert(type(self._acquire_use) == "function", "ModelCatalog acquire_use must be a function")
  self:_refresh_source_fingerprint()

  local restored
  if self._store then
    local ok, value, read_err = pcall(self._store.read, self._store, self._provider_id)
    if ok and value then
      local reason
      restored, reason = cache_record(self._provider_id, value, self._source_fingerprint)
      if not restored then
        self:_diagnose(
          "ignored invalid model catalog cache for " .. self._provider_id,
          util.error("state_store", reason or "cache record is invalid")
        )
      elseif self._discover and #restored.models == 0 then
        self:_diagnose(
          "ignored empty model catalog cache for " .. self._provider_id,
          util.error("state_store", "cache record contains no models")
        )
        restored = nil
      end
    elseif not ok or read_err then
      self:_diagnose("failed to read model catalog cache for " .. self._provider_id, ok and read_err or value)
    end
  end
  local initial = restored and restored.models or definition.seed or {}
  local initial_source = restored and "cache" or fallback_source(self)
  ...
  if not published and restored then
    self:_diagnose("ignored unusable model catalog cache for " .. self._provider_id, publish_err)
  ...
  if type(provider.auth) == "string" and type(self._auth.subscribe) == "function" then
    self._auth_unsubscribe = self._auth:subscribe(provider.auth, function(event)
      self:_auth_changed(event)
    end)
  end
  return self
end
