...
local function safe_text(value)
  return type(value) == "string" and util.is_valid_utf8(value) and not value:find("[%z\1-\31\127]")
end
...
local function copy_json(value, stack)
  ...
  if kind == "string" then
    if not safe_text(value) then
      error("source option text is unsafe", 0)
    end
    return value
  end
  if kind == "number" then
  ...
  if stack[value] then
    error("source options must not contain cycles", 0)
  end
  stack[value] = true
  local result = {}
  if util.is_list(value) then
    for index = 1, #value do
      result[index] = copy_json(value[index], stack)
    end
  else
...
function M.provider_projection(provider)
  ...
  for _, key in ipairs({
    ...
    if provider[key] ~= nil then
      result[key] = util.copy(provider[key])
    end
  end
  return result
end
...
local function source_options(provider, definition)
  if type(definition.source_options) ~= "function" then
    return ""
  end
  local ok, value = pcall(definition.source_options, M.provider_projection(provider))
  if not ok then
    return nil, util.error("provider", "Model catalog source_options failed")
  end
  local copied_ok, copied = pcall(copy_json, value, {})
  if not copied_ok then
    return nil, util.error("provider", "Model catalog source_options returned an invalid value", copied)
  end
  local encoded_ok, encoded = pcall(util.json_encode, copied)
  if not encoded_ok or #encoded > MAX_SOURCE_OPTIONS_BYTES then
    return nil, util.error("provider", "Model catalog source_options exceeds its safe bound")
  end
  return encoded
end
...
local function account_identity(opts, provider, definition)
  ...
  if identity == nil then
    return nil, err or util.error("auth", "Model catalog account identity is unavailable")
  end
  if not safe_text(identity) or identity == "" or #identity > 1024 then
    return nil, util.error("auth", "Model catalog account identity is invalid")
  end
  return identity
end
...
function M.fingerprint(opts)
  ...
  if type(source_id) ~= "string" or source_id == "" or source_revision == nil then
    return nil, util.error("provider", "Model catalog persistence requires source_id and source_revision")
  end
