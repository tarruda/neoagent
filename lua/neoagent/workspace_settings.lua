...
function Settings:load()
  ...
  if not content then
    return nil, settings_error("Failed to read workspace settings", read_err)
  end
...
local function encoded_settings(settings)
  assert(
    type(settings) == "table" and (next(settings) == nil or not util.is_list(settings)),
...
local function prune_agent_scopes(settings)
  ...
  if not object(agents) then
    return settings
  end
  for name, scope in pairs(agents) do
    if object(scope) and next(scope) == nil then
      agents[name] = nil
    end
  end
...
local function replace(self, settings, encoded)
  local ok, err, stage = fs.atomic_replace(self.settings_path, encoded .. "\n", { mode = 384 })
  if not ok then
    local action = stage == "temporary" and "create workspace settings temporary file"
...
local function with_lock(self, fn)
  ...
  if not result and type(err) == "table" and err.kind == "file_lock" then
    ...
    return nil,
      settings_error("Failed to " .. action .. " workspace settings lock", rawget(err, "detail") or err.message)
  end
  return result, err
end
...
function Settings:update(patch)
  assert(
    type(patch) == "table" and (next(patch) == nil or not util.is_list(patch)),
