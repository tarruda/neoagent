...
local function validate_operation(id, value)
  ...
  if not ok_label then
    return nil, err_label
  end
  if value.description ~= nil then
    local ok_description, err_description = valid_text(value.description, "operation " .. id .. " description", 512)
    if not ok_description then
      return nil, err_description
    end
  end
  ...
  if value.auth_scope ~= nil then
    local ok_scope, err_scope = valid_text(value.auth_scope, "operation " .. id .. " auth_scope", 128)
    if not ok_scope then
      return nil, err_scope
    end
...
function M.validate(value)
  ...
  if type(value.operations) ~= "table" or (next(value.operations) ~= nil and util.is_list(value.operations)) then
  ...
  for _, method in ipairs({
    ...
    if value[method] ~= nil and type(value[method]) ~= "function" then
      return failure("Provider Service " .. value.id .. " " .. method .. " must be a function")
    end
  end
  return value
end
...
function M.operations(service)
  ...
  local ids = {}
  for id in pairs(service.operations) do
    ids[#ids + 1] = id
  end
  table.sort(ids)
  local result = {}
...
local function subscriber_failure(err)
  local message = util.text_from_bytes(util.normalize_error(err, "provider").message)
...
local function finish_retirement(state)
  if not state.retiring or state.destroyed or state.users > 0 or state.operation_count > 0 then
    return false
  end
...
function M.subscribe(service, listener, opts)
  service = M.assert(service)
  assert(type(listener) == "function", "Provider Service runtime listener must be a function")
  opts = opts or {}
  assert(
  ...
  assert(
    opts.report == nil or type(opts.report) == "function",
...
function M.acquire_use(service)
  ...
  if state.mutating then
    return failure("Cannot acquire provider use during a mutating provider operation")
  end
...
function M.begin_operation(service, opts)
  ...
  assert(opts.mutating == nil or type(opts.mutating) == "boolean", "provider operation mutating must be a boolean")
  local state = runtime(service)
  if state.retiring then
    return failure("Provider Service is retiring")
  end
  ...
  if mutating then
    if state.users > 0 then
      return failure("Cannot run a mutating provider operation during active provider use")
    end
  ...
  function token:finish()
    ...
    if state.operations[id] ~= self then
      return false
    end
    self.active = false
    state.operations[id] = nil
    state.operation_count = state.operation_count - 1
    if state.mutating == self then
      state.mutating = nil
    end
...
function M.retire(service, destroy)
  service = M.assert(service)
  assert(type(destroy) == "function", "Provider Service retirement requires a destroy callback")
  local state = runtime(service)
  if state.retiring then
    return false
  end
...
function M.public_config(provider)
  ...
  if type(provider.api) == "string" then
...
function M.resolve_auth(opts)
  ...
  return async.run(
    ...
    function()
      ...
      assert(
        type(opts.manager) == "table" and type(opts.manager.resolve) == "function",
...
function M.no_interact()
  ...
  local function unavailable(_, done)
    done.reject(util.error("provider", "Provider interaction is unavailable"))
  end
  return {
    select = unavailable,
    input = unavailable,
...
function M.run(service, operation_id, opts)
  ...
  if
    type(opts.args) == "string"
    and (#opts.args > 16384 or not util.is_valid_utf8(opts.args) or opts.args:find("[%z\1-\31\127]"))
  then
  ...
  token, token_err = claim_operation(token, service, descriptor.mutating == true)
  if not token then
    return nil, token_err
  end

  local constructed, run = pcall(
    async.run,
    ...
    function()
      ...
      if type(value) ~= "table" or type(value.cancel) ~= "function" then
        error(util.error("provider", "Provider operation " .. operation_id .. " must return a Run"), 0)
      end
      return value:await()
    end,
    {
  ...
  if not constructed then
    token:finish()
    return failure("Failed to construct provider operation Run: " .. util.normalize_error(run, "provider").message)
  end
  token._run = run
  if run:is_done() then
    token:finish()
