...
local function cmd_command_index(argv)
  ...
  if not basename or basename:lower() ~= "cmd.exe" then
    return nil
  end
  for index = 2, #argv do
...
function M.spawn(command, opts, on_exit)
  local command_index = cmd_command_index(command)
  if not command_index or command_index ~= #command - 1 then
    return vim.system(command, opts, on_exit)
  end
  local argv = util.copy(command)
  ...
  -- The outer pair belongs to /c; the command keeps its own inner quotes.
  argv[#argv] = '"' .. argv[#argv] .. '"'
