...
local function check_string(value, label, maximum, allow_empty)
  if type(value) ~= "string" or not allow_empty and value == "" then
    return failure(label .. (allow_empty and " must be a string" or " is required"))
  end
...
local function json_value(value, label, stack, depth, count)
  ...
  if kind == "number" then
    if not finite(value) then
      return failure(label .. " must be finite")
    end
    return true
  end
  ...
  if kind ~= "table" then
    return failure(label .. " must contain JSON values")
  end
  if depth >= 32 then
    return failure(label .. " exceeds the nesting limit")
  end
  if stack[value] then
    return failure(label .. " must not contain cycles")
  end
  count.value = count.value + 1
  if count.value > 10000 then
    return failure(label .. " exceeds the value limit")
  end
  stack[value] = true
  if util.is_list(value) then
    for item_index, item in ipairs(value) do
      local ok, err = json_value(item, label .. "[" .. tostring(item_index) .. "]", stack, depth + 1, count)
      if not ok then
        stack[value] = nil
        return nil, err
      end
    ...
    for key, item in pairs(value) do
      ...
      local ok, err = json_value(item, label .. "." .. key, stack, depth + 1, count)
      if not ok then
        stack[value] = nil
        return nil, err
      end
    ...
    if key == "cost" then
      ...
      for cost_key, cost in pairs(item) do
        if not finite(cost) or cost < 0 then
          return failure("message usage cost " .. cost_key .. " must be a non-negative finite number")
        end
      end
    elseif not finite(item) or item < 0 then
      return failure("message usage " .. key .. " must be a non-negative finite number")
    end
  end
...
local function normalize_block(value, role, transient)
  if not object(value) then
    return failure("content block must be an object")
  end
  ...
  if block_type == "text" then
    ...
    valid, err = check_optional_string(result.textSignature, "text block signature", 4096)
    if not valid then
      return nil, err
    end
  ...
  elseif block_type == "thinking" then
    if type(result.thinking) ~= "string" or not util.is_valid_utf8(result.thinking) then
    ...
    valid, err = check_optional_string(result.thinkingSignature, "thinking block signature", 1024 * 1024, true)
    if not valid then
      return nil, err
    end
  ...
  elseif block_type == "toolCall" then
    ...
    valid, err = check_optional_string(result.argumentsError, "toolCall argumentsError", 4096)
    if not valid then
      return nil, err
    end
    ...
    if result.revision ~= nil then
      ...
      if revision_type == "string" then
        valid, err = check_string(result.revision, "image revision", MAX_TYPE_BYTES)
        if not valid then
          return nil, err
        end
...
function M.normalize(message)
  ...
  if
    ...
    and (not finite(result.timestamp) or result.timestamp < 0 or result.timestamp % 1 ~= 0)
  then
    return failure("message timestamp must be a non-negative integer")
  end
  if role == "assistant" then
    for _, name in ipairs({ "api", "provider", "model", "stopReason", "responseId" }) do
      valid, err =
        check_optional_string(result[name], "assistant " .. name, name == "responseId" and 4096 or MAX_ID_BYTES)
      if not valid then
        return nil, err
      end
    end
    if result.errorMessage ~= nil then
      if type(result.errorMessage) ~= "string" or not util.is_valid_utf8(result.errorMessage) then
  ...
  elseif role == "toolResult" then
    valid, err = check_string(result.toolCallId, "toolResult toolCallId", MAX_ID_BYTES)
    if not valid then
      return nil, err
    end
    valid, err = check_optional_string(result.toolName, "toolResult toolName", MAX_ID_BYTES)
    if not valid then
      return nil, err
    end
    ...
    return nil,
      {
        ...
        message = "Invalid assistant message: " .. err,
        detail = err,
      }
  end
  if result.role ~= "assistant" then
    return nil, { kind = "protocol", code = "invalid_assistant_message", message = "assistant message is required" }
  end
...
function M.normalize_partial_assistant(message)
  ...
  for _, block in ipairs(message.content) do
    if type(block) == "table" then
      ...
      local meaningful = retained.type == "text" and type(retained.text) == "string" and retained.text ~= ""
        or retained.type == "thinking" and type(retained.thinking) == "string" and retained.thinking ~= ""
...
function M.normalize_list(messages, opts)
  ...
  assert(opts == nil or type(opts) == "table", "message normalization options must be an object")
  local index_offset = opts and opts.index_offset or 0
  assert(
    type(index_offset) == "number" and index_offset >= 0 and index_offset % 1 == 0,
  ...
  for message_index, message in ipairs(messages) do
      ...
      for _, block in ipairs(normalized.content) do
        if block.type == "toolCall" then
          if seen_calls[block.id] then
            return failure(
              "message " .. tostring(displayed_index) .. ": duplicate conversation toolCall id: " .. block.id
            )
          end
    ...
    elseif normalized.role == "toolResult" then
      ...
      if name == nil then
        return failure(
          "message "
            .. tostring(displayed_index)
            .. ": toolResult references an unknown toolCall: "
            .. normalized.toolCallId
        )
      end
      if normalized.toolName ~= nil and normalized.toolName ~= name then
        return failure("message " .. tostring(displayed_index) .. ": toolResult toolName does not match its toolCall")
      end
...
function M.normalize_tool_result(result, opts)
  ...
  normalized.content, err = normalize_content(normalized.content, "toolResult", opts and opts.transient == true)
  if not normalized.content then
    return nil, err
  end
  if
    normalized.isError ~= nil and type(normalized.isError) ~= "boolean"
    or normalized.is_error ~= nil and type(normalized.is_error) ~= "boolean"
  then
  ...
  if result.usage ~= nil and normalized.usage == nil then
    return nil, err
  end
