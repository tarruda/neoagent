...
local function valid_env(env)
  ...
  if type(env) ~= "table" or util.is_list(env) then
    return false
  end
...
local function safe_cache_identity(value)
  ...
  then
    return nil, util.error("auth", "Login method cache_identity must return safe non-empty text")
  end
  return value
end
...
local function hash_identity(method_id, value)
  return vim.fn.sha256(#method_id .. ":" .. method_id .. ":" .. #value .. ":" .. value)
end
...
local function credential_type(credential)
  ...
  if credential.type == "api_key" then
    return "api_key"
  end
  if credential.type == "oauth" then
    return "oauth"
  end
  if
    credential.type == nil
    and type(credential.access) == "string"
    and type(credential.refresh) == "string"
...
local function valid_credential(credential)
  ...
  if kind == "api_key" then
    return type(credential.key) == "string" and util.trim(credential.key) ~= "" and valid_env(credential.env)
  end
  if kind == "oauth" then
    return type(credential.access) == "string"
...
local function valid_for(credential, method)
  if not valid_credential(credential) or method.type ~= nil and method.type ~= credential_type(credential) then
    return false
  end
  if type(method.validate_credential) ~= "function" then
    return true
  end
...
local function method_for(self, id)
  ...
  if not method then
    error(util.error("auth", "Unknown login method: " .. tostring(id)), 0)
  end
  return method
end
...
local function credential_identity(self, id, method, credential)
  ...
  if not ok then
    return nil, util.error("auth", "Login method cache_identity failed")
  end
...
local function credential_from(result, action, method)
  ...
  if not result.ok then
    error(result.error or util.error("auth", action .. " failed"), 0)
  end
...
function Manager:login(id, opts)
  ...
  return async.run(
    ...
    function(run)
      ...
      local stored = modify_store(self, id, function()
        return credential
      end)
    ...
    { on_event = opts.on_event, on_done = opts.on_done, error_kind = "auth" }
  )
end
  ...
  return async.run(
    ...
    function()
      if
        opts.scope ~= nil
        and (
          type(opts.scope) ~= "string"
          or opts.scope == ""
          or #opts.scope > 128
          or not opts.scope:match("^[%w_.-]+$")
        )
      then
      ...
      if credential_type(credential) == "oauth" and self.now() >= credential.expires then
        ...
        credential = modify_store(self, id, function(current)
          ...
          if credential_type(current) ~= "oauth" or self.now() < current.expires then
            return nil
          end
          return credential_from(method.refresh(util.copy(current)):await(), "Token refresh", method)
        end)
        if not credential then
          error(util.error("auth", "Credential was removed during refresh"), 0)
        end
        if not valid_for(credential, method) then
        ...
        if previous_identity ~= next_identity and (previous_identity ~= nil or next_identity ~= nil) then
          publish(self, id, "refresh")
        end
      end
      local override = method.request_opts(util.copy(credential), opts.scope)
    ...
    { on_done = opts.on_done, error_kind = "auth" }
  )
end
...
function Manager:derive_cache_identity(id, credential)
  local method = method_for(self, id)
  if not valid_for(credential, method) then
    return nil, util.error("auth", "Credential is invalid for cache identity: " .. id)
  end
...
function Manager:subscribe(id, listener)
  method_for(self, id)
  assert(type(listener) == "function", "authentication revision listener must be a function")
  ...
  return function()
    ...
    if self.listeners[id] then
      self.listeners[id][listener_id] = nil
    end
    return true
  end
end
  ...
  for _, entry in ipairs(entries) do
    ...
    if type(id) == "string" and id ~= "" then
      ...
      result[#result + 1] = {
        ...
        type = entry.type == "api_key" and "api_key" or entry.type == "oauth" and "oauth" or "invalid",
      }
    end
  end
...
function M.new(opts)
  ...
  assert(
    type(opts.store) == "table" and type(opts.store.read) == "function" and type(opts.store.write) == "function",
