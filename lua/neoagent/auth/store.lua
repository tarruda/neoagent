...
function Store:_lock()
  ...
  if not ok then
    error(credential_lock_error(lease, false), 0)
  end
  return function()
    local released, release_err = lease:release()
    if not released then
      return nil, credential_lock_error(release_err, true)
    end
    return true
  end
end
...
function Store:_read_all()
  ...
  if not content then
    return nil, failure("Failed to read credentials", err)
  end
...
function Store:list()
  ...
  for id, credential in pairs(values) do
    ...
    if kind == nil and type(credential) == "table" and credential.access ~= nil then
      kind = "oauth"
    end
...
function Store:_write_all(values)
  ...
  if not ok then
    return nil, failure("Failed to create credential directory", err)
  end
  local encoded = next(values) == nil and vim.empty_dict() or values
  local stage
  ok, err, stage = fs.atomic_replace(self.path, vim.json.encode(encoded) .. "\n", { mode = 384 })
  if not ok then
...
function Store:write(id, credential)
  ...
  if not created then
    return nil, failure("Failed to create credential directory", create_err)
  end
...
function Store:delete(id)
  ...
  return self:modify(id, function()
    return DELETE
  end)
end
...
function Store:modify(id, fn)
  ...
  return async.run(
    ...
    function()
      ...
      local created, create_err = fs.ensure_private_directory(directory, 448)
      if not created then
        error(failure("Failed to create credential directory", create_err), 0)
      end
...
function M.new(path, opts)
  ...
  opts = opts or {}
  local lock_timeout_ms = positive_integer(opts.lock_timeout_ms or DEFAULT_LOCK_TIMEOUT_MS, "lock_timeout_ms")
  local lock_poll_ms = positive_integer(opts.lock_poll_ms or DEFAULT_LOCK_POLL_MS, "lock_poll_ms")
  return setmetatable({
