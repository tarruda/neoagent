...
local function exact(message, allowed)
  assert(validation.object(message), "RPC artifact event must be an object")
  validation.exact(message, allowed, "RPC artifact event")
end
...
function Importer:accept(message)
  ...
  if message.type == "artifact_chunk" then
    ...
    assert(
      type(message.data) == "string" and message.data ~= "" and #message.data <= limits.MAX_ARTIFACT_CHUNK_BYTES,
