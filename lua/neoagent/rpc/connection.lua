...
-- Connection transitions and receipt invariants:
-- new -> waiting_ready -> opening -> open -> closing -> closed
-- open -> request -> open; request -> cancelling -> open
-- Any state, including closed during final stream validation, can fail.
--
-- Receipt flags advance in feed(), before a scheduled waiter resumes. They
-- reject duplicate handshakes/terminals even when earlier input is still queued.
-- A request remains active through event validation after terminal receipt.
-- Cancellation before a terminal drains protocol traffic in cancelling;
-- cancellation after receipt leaves validation with the request coroutine.
-- Failure preserves queued input only for an already received terminal, and
-- never permits another request. Neither close nor failure disposes the lease.
...
---@class Neoagent.RpcTransport
---@field write fun(self: Neoagent.RpcTransport, bytes: string): true?, Neoagent.Error?
---@field close_stdin fun(self: Neoagent.RpcTransport): true?, Neoagent.Error?

---@class Neoagent.RpcConnection
---@field _transport? Neoagent.RpcTransport
---@field _decoder Neoagent.FrameDecoder
---@field _queue table<integer, Neoagent.RpcQueuedMessage>
---@field _queue_head integer
---@field _queue_tail integer
...
---@field _preserve_terminal? boolean
...
local function fail(self, err)
  ...
  -- A later channel failure cannot revoke a received terminal response.
  -- Request events and the domain result still undergo their normal validation.
  self._preserve_terminal = self._active ~= nil and self._active.terminal_received or nil
  self._state = "failed"
  ...
  if active and not active.terminal_received and active.cancel then
    -- A request can be suspended inside its asynchronous event validator,
    -- independently of the channel's message waiter. Revoke that work too.
    active.cancel()
  end
...
---@param self Neoagent.RpcConnection
local function settle_request(self)
  self._queue = {}
  self._queue_head = 1
  self._queue_tail = 0
...
local function begin_cancel(self, request_id)
  ...
  if active.terminal_received then
    -- The request coroutine still owns validation of queued events and the
...
---@param message table
---@return boolean
local function accept_request_event(self, message)
  local active = self._active
...
---@param self Neoagent.RpcConnection
---@param request_id? integer
local function cancel_request(self, request_id)
  local active = self._active
  if request_id and (not active or active.id ~= request_id) then
    return
  end
...
local function accept_detached_cancel(self, message)
  ...
  if message.type == "event" then
    accept_request_event(self, message)
    return true
  end
...
local function accept_connection_event(self, message)
  ...
  if
    message.call_id ~= self._call_id
    or self._state ~= "open" and self._state ~= "request" and self._state ~= "cancelling"
  then
...
---@param self Neoagent.RpcConnection
---@param message table
---@param frame_bytes integer
local function enqueue(self, message, frame_bytes)
  ...
  elseif state == "request" then
    if active and not active.terminal_received then
      accepted = message.type == "event"
  ...
  if state == "request" and not accept_request_event(self, message) then
    return
  end
  if self._queued_bytes + frame_bytes > protocol.MAX_QUEUED_BYTES then
  ...
  if
    ...
    and (message.type == "response" or message.type == "request_error" or message.type == "cancelled")
  then
  ...
  self._queue[self._queue_tail] = { message = message, bytes = frame_bytes }
  self._queued_bytes = self._queued_bytes + frame_bytes
end
...
local function next_message(self)
  if self._queue_head <= self._queue_tail then
    local queued = assert(self._queue[self._queue_head])
    self._queue[self._queue_head] = nil
    self._queue_head = self._queue_head + 1
    self._queued_bytes = self._queued_bytes - queued.bytes
    if self._queue_head > self._queue_tail then
      self._queue_head = 1
      self._queue_tail = 0
    end
  ...
  if not completed then
    local active = self._active
    if received and active and active.terminal_received and util.normalize_error(value).kind == "cancelled" then
      return received
    end
    error(value, 0)
  end
...
local function request(self, active, payload)
  ...
  local function await_result()
    while true do
      ...
      if self._failure and not self._preserve_terminal then
        raise_failure(self)
      end
      if message.type == "event" then
        ...
        if not accepted then
          ...
          fail(self, err.kind == "protocol" and protocol_error("RPC event handler failed: " .. err.message) or err)
          raise_failure(self)
        end
  ...
  if not completed then
    if self._failure and not self._preserve_terminal then
      -- Cancelling event processing wakes its await with cancellation. The
      -- request still belongs to the channel failure that interrupted it.
      raise_failure(self)
    end
    if self._active == active and active.terminal_received and util.normalize_error(value).kind == "cancelled" then
      settle_request(self)
    end
    error(value, 0)
  end
...
---@param transport Neoagent.RpcTransport
function RpcConnection:attach(transport)
  assert(self._transport == nil and type(transport) == "table", "RPC channel is already attached")
  assert(type(transport.write) == "function", "RPC channel must provide write")
  assert(type(transport.close_stdin) == "function", "RPC channel must provide close_stdin")
...
function RpcConnection:feed(chunk)
  ...
  if self._state == "closed" then
    if chunk ~= "" then
      fail(self, protocol_error("RPC peer sent data after acknowledging shutdown"))
    end
    return
  end
...
function RpcConnection:eof(result)
  ...
  local kind = starting and "worker_start" or state == "cancelling" and "cancelled" or "protocol"
  ...
  local detail = result.stderr ~= ""
      and util.safe_message(result.stderr, {
        max_characters = protocol.MAX_ERROR_BYTES,
        max_source_bytes = protocol.MAX_ERROR_BYTES,
      })
    or nil
...
function RpcConnection:open(context)
  if self._state ~= "new" or not self._transport then
    if self._failure then
      raise_failure(self)
    end
  ...
  local ready = next_message_bounded(self, self._start_grace_ms, "worker_start", "RPC peer did not announce readiness")
  if self._failure then
    raise_failure(self)
  end
  ...
  local opened =
    next_message_bounded(self, self._start_grace_ms, "worker_start", "RPC peer did not acknowledge the connection")
  if self._failure then
    raise_failure(self)
  end
...
function RpcConnection:close()
  ...
  local closed =
    next_message_bounded(self, self._shutdown_grace_ms, "protocol", "RPC peer did not acknowledge shutdown")
  if self._failure then
    raise_failure(self)
  end
  ...
  self._state = "closed"
  local transport = assert(self._transport)
  if not self._exit then
    local completed, stdin_closed, close_err = pcall(transport.close_stdin, transport)
    if not completed or not stdin_closed then
      fail(
        self,
        completed and (close_err or protocol_error("Could not close RPC channel input")) or protocol_error(stdin_closed)
      )
      raise_failure(self)
    end
  end
...
---@param err? Neoagent.Error
function RpcConnection:abort(err)
  fail(self, self._failure or err or util.error("protocol", "RPC connection was aborted"))
end

---@return boolean
function RpcConnection:is_failed()
  return self._failure ~= nil
end
...
function RpcCall:result()
  local completed, value = pcall(self._run.await, self._run)
  if not completed then
    -- The child may have finished validating a received response while the
    -- cancelled caller was waiting for its scheduled delivery.
    local result = self._run:result()
    if util.normalize_error(value).kind ~= "cancelled" or not result then
      error(value, 0)
    end
    value = result
  end
...
function M.new(opts)
  ...
  client._decoder = protocol.decoder(function(message, frame_bytes)
    enqueue(client, message, frame_bytes)
  end)
  return client
end
