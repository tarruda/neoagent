...
---@field owns_presenter? boolean Own and destroy the injected Presenter.
...
local function validate_owner_callbacks(callbacks)
  assert(
    type(callbacks) == "table" and (next(callbacks) == nil or not util.is_list(callbacks)),
    "Agent Applet owner callbacks must be an object"
  )
  for name, callback in pairs(callbacks) do
    assert(owner_callback_names[name], "Unknown Agent Applet owner callback: " .. tostring(name))
    assert(type(callback) == "function", "Agent Applet " .. name .. " must be a function")
  end
end

---@param agent Neoagent.Agent
local function assert_agent(agent)
  assert(type(agent) == "table" and agent._neoagent_agent, "Agent Applet requires a Neoagent Agent")
end
...
local function call_view(view, method, ...)
  ...
  if not ok then
    return nil, util.normalize_error(value, "ui")
  end
...
local function valid_snapshot(snapshot)
  return type(snapshot) == "table"
    ...
    and type(snapshot.messages) == "table"
    and util.is_list(snapshot.messages)
    and type(snapshot.context) == "table"
    and (next(snapshot.context) == nil or not util.is_list(snapshot.context))
    and type(snapshot.context.workspace) == "string"
    and snapshot.context.workspace ~= ""
    and type(snapshot.events) == "table"
    and util.is_list(snapshot.events)
end
...
function AgentApplet.new(opts)
  opts = opts or {}
  assert(type(opts.config) == "table", "Agent Applet UI config is required")
  assert(
    opts.context == nil or type(opts.context) == "table" and not util.is_list(opts.context),
    "Agent Applet context must be an object"
  )

  assert(opts.owns_presenter == nil or type(opts.owns_presenter) == "boolean", "Presenter ownership must be a boolean")
  local owns_presenter = opts.presenter == nil or opts.owns_presenter == true
...
function AgentApplet:claim(owner, callbacks)
  ...
  assert(self.owner_value == nil, "Agent Applet already has an owner")
...
function AgentApplet:release(owner)
  assert(self.owner_value == owner, "Agent Applet is not owned by the caller")
...
function AgentApplet:_set_view_presentation(view)
  local ok, shown, err = pcall(view.set_presentation, view, self.presentation)
  if ok and shown ~= false and err == nil then
    return true
  end
...
function AgentApplet:_set_view_dialog(view)
  ...
  self.dialog_source:cancel_pending("Dialog presentation failed: " .. failure.message, unavailable)
  return nil, failure
end
...
function AgentApplet:_history_store(root)
  ...
  if not persistence or not persistence.enabled then
    return nil
  end
...
function AgentApplet:_load_history(root, refresh)
  if self.histories[root] and not refresh then
  ...
  if not history then
    assert(err)
    self:_notify(err.message .. (err.detail and ": " .. err.detail or ""), vim.log.levels.WARN)
    self.histories[root] = self.histories[root] or {}
    return self.histories[root]
  end
...
---@param root string
function AgentApplet:_bind_workspace(root)
  assert(type(root) == "string" and root ~= "", "Agent Applet Workspace is required")
  assert(self.workspace_root == nil or self.workspace_root == root, "Agent Applet Workspace is immutable")
...
function AgentApplet:_record_history(text)
  local root = assert(self.workspace_root, "Agent Applet Workspace is required")
  text = util.trim(text)
  assert(text ~= "", "accepted submission text must not be empty")
  local store = self:_history_store(root)
  if not store then
  ...
  if not history then
    assert(err)
    self:_notify("input history was not saved: " .. err.message, vim.log.levels.WARN)
    return nil, err
  end
  self.histories[root] = history
  return true
...
function AgentApplet:_context_for(value, bound, label)
  value = bound and util.copy(value or {}) or util.deep_merge(self.draft_context, value or {}) --[[@as Neoagent.AgentContext]]
...
function AgentApplet:_capture_input(view)
  ...
  if ok and type(value) == "string" then
    if value ~= self.input_value then
...
function AgentApplet:_hydrate_view(view, snapshot, label, files)
  ...
  local applied, err
  if positions[snapshot.context.position] then
    applied, err = call_view(view, "set_position", snapshot.context.position)
    if not applied then
      return nil, err
    end
  end
  ...
  return {
    ...
    workspace = assert(snapshot.context.workspace),
    position = positions[snapshot.context.position] and snapshot.context.position or nil,
  }
end
...
function AgentApplet:set_draft_context(patch)
  assert(type(patch) == "table" and not util.is_list(patch), "Agent Applet draft context must be an object")
  if self:_agent_or_nil() then
    return nil, util.error("agent", "A bound Agent owns the Applet context")
  end
  assert(patch.workspace == nil or patch.workspace == self.workspace_root, "Agent Applet Workspace is immutable")
...
function AgentApplet:_sync_position(context, view)
  ...
  if not positions[position] or position == self.position then
    return false
  end
  assert(position)
...
function AgentApplet:_apply(update)
  ...
  assert(
    type(update.revision) == "number" and update.revision >= 1 and update.revision % 1 == 0,
    "Agent publication revision must be a positive integer"
  )
  if update.revision <= (snapshot.revision or 0) then
    return false
  end
  self.agent_snapshot = snapshot
  snapshot.revision = update.revision
  if update.type == "context" then
    assert(update.context.workspace == self.workspace_root, "Agent publication changed the Applet Workspace")
  ...
  elseif update.type == "event" then
    ...
    elseif
      ...
      and update.event.type ~= "inference_stats"
    then
  ...
  elseif update.type == "event" then
    if update.event.type ~= "inference_stats" then
...
function AgentApplet:_ensure_agent(value)
  ...
  if not agent then
    ...
    if not agent then
      return nil, util.error("agent", "Agent construction did not bind the Applet")
    end
    created = true
  end
...
function AgentApplet:_queue_submission(agent, text, created, submission_id, kind)
  assert(
    type(submission_id) == "number" and submission_id >= 1 and submission_id % 1 == 0,
    "Agent submission id must be a positive integer"
  )
  assert(kind == "turn" or kind == "steering", "Agent submission kind is invalid")
  local submission = {
    id = submission_id,
    agent = agent,
    text = text,
...
function AgentApplet:_clear_submission_input(submission)
  if submission.kind ~= "steering" or self.input_submission_id ~= submission.id then
    return false
  end
  local view = self.view_value
  local current = self:_capture_input(view)
  if current ~= submission.text then
    return false
  end
  if view and not view.destroyed then
...
function AgentApplet:_forget_submissions(ids)
  ...
  for _, submission in ipairs(self.submissions) do
    if not selected[submission.id] then
      retained[#retained + 1] = submission
    end
  end
  self.submissions = retained
  if selected[self.input_submission_id] then
    self.input_submission_id = nil
  end
end
...
function AgentApplet:_submission_accepted(update)
  ...
  if not selected or selected.agent ~= self:_agent_or_nil() then
    return false
  end
  table.remove(self.submissions, index)
  if selected.created then
    self:_accept_agent(assert(selected.agent))
  end
  self.pending_submission = nil
  local view = self.view_value
  local current = self:_capture_input(view)
  if self.input_submission_id == selected.id then
    self.input_submission_id = nil
    if current == selected.text then
      self.input_value = ""
    end
  end
  if view and not view.destroyed then
    local accepted, err = call_view(view, "submission_accepted", selected.text)
...
function AgentApplet:_finish_submissions(result)
  ...
  for _, submission in ipairs(self.submissions) do
    ...
    elseif submission.kind == "steering" then
      retained[#retained + 1] = submission
      if not result.ok and submission.input_cleared and self.input_submission_id == submission.id then
        restore = submission
      end
    end
  end
...
function AgentApplet:_accept_agent(agent)
  if self:_agent_or_nil() ~= agent or not self.binding_restore then
    return false
  end
...
function AgentApplet:_attempt_submission(text)
  ...
  self:_restore_input(text)
  local represented = self.input_submission_id and self:_submission(self.input_submission_id) or nil
  if
    represented
    and represented.agent == agent
    and represented.kind == "turn"
    and represented.text == text
  ...
  if not prepared then
    if prepare_err and prepare_err.kind == "workspace_trust" and rawget(prepare_err, "pending") == true then
  ...
  if represented and represented.agent == agent and represented.kind == "steering" then
  ...
  if run then
    self.pending_submission = nil
    local submission = self:_queue_submission(agent, text, provisional, assert(submission_id), (assert(kind)))
  ...
  elseif err and err.kind == "workspace_trust" and rawget(err, "pending") == true then
    ...
    self.pending_submission = nil
    if provisional then
      return self:_reject_agent(agent, err, text)
    end
  end
  return run, err
end
...
function AgentApplet:retry_submission()
  ...
  if not text or not self:_agent_or_nil() then
    return false
  end
...
function AgentApplet:trust_submission_result(result)
  local text = self.pending_submission
  local agent = self:_agent_or_nil()
  if not text or not agent then
    return false
  end
  if result.ok then
    return self:retry_submission()
  end
  self.pending_submission = nil
...
function AgentApplet:_ensure_view()
  ...
  local built, staged = pcall(function()
    candidate = factory({
      ...
      on_input_history = function()
        return self:input_history()
      end,
      on_select_history = function()
        return self:select_input_history()
      end,
      ...
      on_close = function()
        if not self.destroyed then
          self:_owner_callback("on_close")
        end
      end,
    })
    assert(type(candidate) == "table", "View factory must return a View")
    for _, method in ipairs(required_view_methods) do
      assert(type(candidate[method]) == "function", "View must implement " .. method)
    end
    local applied, err = call_view(candidate, "set_input", self.input_value)
    if not applied then
      error(err, 0)
    end
    applied, err = call_view(candidate, "set_context", self:_context_for(nil, false))
    if not applied then
      error(err, 0)
    end
    if self.dialog then
      applied, err = call_view(candidate, "set_dialog", util.copy(self.dialog))
      if not applied then
        error(err, 0)
      end
    end
    if self.presentation then
      applied, err = call_view(candidate, "set_presentation", util.copy(self.presentation))
      if not applied then
        error(err, 0)
      end
    end
    ...
    local hydrated
    if agent then
      local snapshot = util.copy(self.agent_snapshot)
      ...
      while true do
        ...
        if not newest or newest.revision <= snapshot.revision then
          break
        end
        snapshot = util.copy(newest)
      end
    end
    return { view = candidate, hydrated = hydrated }
  end)
  if not built then
    if candidate and type(candidate.destroy) == "function" and not candidate.destroyed then
  ...
  if staged.hydrated then
    ...
    self:_bind_workspace(assert(staged.hydrated.workspace))
...
function AgentApplet:bind(agent, opts)
  ...
  assert(
    opts.provisional == nil or type(opts.provisional) == "boolean",
    "Agent Applet bind provisional must be boolean"
  )
  assert(self.agent_value == nil, "Agent Applet is already bound")
  assert(agent:presenter() == self.presenter_source, "Agent and Applet must share one Presenter")
  assert(agent:dialogs() == self.dialog_source, "Agent and Applet must share one Dialog source")
  assert(agent:applet() == nil or agent:applet() == self, "Agent already owns another Applet")
  local previous = {
  ...
  local unsubscribe = agent:subscribe(function(update)
    if pending then
      pending[#pending + 1] = util.copy(update)
    else
      self:_apply(update)
    end
  end)
  ...
  local function binding_error(value)
    unsubscribe()
    if attached then
      pcall(agent.detach_applet, agent, self)
  ...
  local ok, failure = pcall(function()
    local snapshot = agent:snapshot()
    local workspace = snapshot.context.workspace
    assert(type(workspace) == "string" and workspace ~= "", "Agent Applet Workspace is required")
    assert(self.workspace_root == nil or self.workspace_root == workspace, "Agent Applet Workspace is immutable")
    ...
    if view and not view.destroyed then
      ...
      local err
      local prepared
      prepared, err = self:_hydrate_view(view, snapshot, agent:label(), agent:get_session():files())
      if not prepared then
        error(err, 0)
      end
      hydrated = prepared
    end
    assert(not self.destroyed, "Agent Applet was destroyed during binding")
    self:_bind_workspace(hydrated.workspace)
    agent:attach_applet(self)
    attached = true
    agent:set_attention("dialog", dialog_attention(self.dialog))
    agent:set_attention("presentation", presentation_attention(self.presentation))
    self.agent_value = agent
    self.display_label = agent:label()
    self.agent_snapshot = hydrated.snapshot
    self.binding_restore = opts.provisional
        and {
          display_label = previous.display_label,
          agent_snapshot = previous.agent_snapshot,
        }
      or nil
    if hydrated.position then
      self.position = hydrated.position
    end
    self.agent_unsubscribe = unsubscribe
    -- Keep queuing during replay so reentrant publications cannot overtake
    -- earlier updates from hydration.
    for _, update in ipairs(assert(pending)) do
      self:_apply(update)
    end
    pending = nil
  end)
  if not ok then
    error(binding_error(failure).message, 0)
  end
  return agent
end
...
function AgentApplet:unbind(agent)
  assert(not self.destroyed, "Agent Applet is destroyed")
  assert(self.agent_value == agent, "Agent Applet is not bound to the caller")
  pcall(agent.set_attention, agent, "dialog", nil)
  pcall(agent.set_attention, agent, "presentation", nil)
  local view = self.view_value
  self:_capture_input(view)
  if self.agent_unsubscribe then
    pcall(self.agent_unsubscribe)
  end
  self.agent_unsubscribe = nil
  ...
  if not detached then
    self:_notify(util.normalize_error(detach_err, "agent").message, vim.log.levels.ERROR)
  end
  if not detached then
    return agent, util.normalize_error(detach_err, "agent")
  end
  return agent
end

---@return Neoagent.Agent?
function AgentApplet:agent()
  return self:_agent_or_nil()
end
---@return Neoagent.Presenter
function AgentApplet:presenter()
  return self.presenter_source
end
---@return Neoagent.Dialogs
function AgentApplet:dialogs()
  return self.dialog_source
end
---@return Neoagent.View?
function AgentApplet:view()
...
function AgentApplet:open(opts)
  opts = opts or {}
  assert(type(opts) == "table", "Agent Applet open options must be a table")
  ...
  if agent then
    ...
    if not prepared and not (err and err.kind == "workspace_trust" and rawget(err, "pending") == true) then
      return nil, err
    end
  end
  local ready, view = pcall(self._ensure_view, self)
  if not ready then
    return nil, util.normalize_error(view, "ui")
  end
...
---@return boolean
function AgentApplet:is_destroyed()
...
function AgentApplet:focus_input()
  local view = self.view_value
  return view and view:focus_input() or false
end
...
function AgentApplet:focus_attention()
  ...
  if self.presentation and self.presentation.active then
    local key = self.presentation.active.kind == "select" and "presentation-filter" or "presentation"
    local pane = view:pane(key)
    return pane and pane:focus() or false
  end
  if self.dialog and self.dialog.active then
    local pane = view:pane("dialog")
    if pane then
      return pane:focus()
    end
    return view:focus_transcript()
  end
  return view:focus_input()
end
...
function AgentApplet:input_history()
  ...
  return util.copy(self:_load_history(assert(self.workspace_root, "Agent Applet Workspace is required"), persistent))
end
...
function AgentApplet:set_position(position)
  ...
  local err
  if agent then
    saved, err = agent:set_ui_position(position)
  end
  local view = self.view_value
  if view and not view.destroyed then
    view:set_position(position)
  ...
  if not saved then
    assert(err)
    self:_notify("window position changed but workspace settings were not saved: " .. err.message, vim.log.levels.WARN)
  end
  return position, err
end
...
function AgentApplet:destroy()
  ...
  if self.dialog_unsubscribe then
    pcall(self.dialog_unsubscribe)
  end
  if self.presenter_unsubscribe then
    pcall(self.presenter_unsubscribe, self.owns_presenter and "Agent Applet was destroyed" or nil)
  end
  self.agent_unsubscribe = nil
