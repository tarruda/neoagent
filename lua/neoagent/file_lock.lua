...
local function backend_failure(err, fallback_code, fallback_message)
  ...
  return failure(err.code or fallback_code, err.message or fallback_message, err.detail)
end
...
function Lease:release()
  ...
  if not verified then
    return nil, backend_failure(verify_err, "ownership", "Failed to verify file lock ownership")
  end
  if not released then
    return nil, backend_failure(release_err, "release", "Failed to unlock file lock")
  end
  if not closed then
    return nil, backend_failure(close_err, "release", "Failed to close file lock")
  end
  return true
end
...
function Lock:_close_state(state)
  ...
  local closed, close_err = handle:close()
  if not closed then
    return nil, backend_failure(close_err, "release", "Failed to close file lock candidate")
  end
  return true
end
...
function Lock:_attempt(state)
  ...
  if not acquired then
    ...
    state.handle = nil
    handle:close()
    return nil, backend_failure(acquire_err, "lock", "Failed to acquire file lock")
  end
  ...
  if prepared then
    written, write_err = state.handle:write_token(state.token)
  end
  if written then
    verified, verify_err = state.handle:verify_token(state.token)
  end
  if not prepared or not written or not verified then
    ...
    return nil, backend_failure(initialization_err, "initialize", "Failed to initialize file lock")
  end
  ...
  return async.await(
    ...
    function(done)
      ...
      local function attempt()
        ...
        timer:start(math.min(self.poll_ms, math.max(1, math.floor(self.timeout_ms - elapsed))), 0, attempt)
      end
...
function M.new(opts)
  opts = opts or {}
  assert(type(opts) == "table" and (next(opts) == nil or not util.is_list(opts)), "file lock options must be an object")
  for name in pairs(opts) do
    assert(
      name == "path" or name == "timeout_ms" or name == "poll_ms" or name == "mode",
