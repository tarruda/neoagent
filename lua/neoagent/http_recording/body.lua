...
function M.new(path, sensitive)
  return setmetatable({
    path = path, sensitive = sensitive, chunks = {}, bytes = 0, count = 0,
  }, Body)
end
...
function Body:append(chunk)
  ...
  if not self.file and self.bytes <= MEMORY_LIMIT then
    ...
    if #self.chunks >= 256 then self.chunks = { table.concat(self.chunks) } end
    return
  end
  if not self.file then
    assert(not self.sensitive, "sensitive response exceeds recording memory limit")
    local created, identity = fs.atomic_replace(
      self.path, table.concat(self.chunks), { mode = 384 })
...
---@return string?
function Body:text()
  if self.file then return nil end
...
function Body:write_base64(write)
  ...
  local read, read_err = file:read_chunks(function(chunk)
    ...
    assert(write(vim.base64.encode(data:sub(1, length))),
      "failed to write recording body")
    carry = data:sub(length + 1)
  end)
  if not read then return nil, read_err end
