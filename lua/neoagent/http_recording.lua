...
local function report(self, message)
  ...
  pcall(self._report, "neoagent: HTTP recording: " .. selected, vim.log.levels.ERROR)
end
...
local function iso_timestamp(milliseconds)
  ...
  return os.date("!%Y-%m-%dT%H:%M:%S", seconds) .. string.format(".%03dZ", milliseconds % 1000)
end
...
local function slug(value, fallback)
  local selected = sanitization.safe_string(value):gsub("[^%w._-]+", "-"):gsub("^-+", ""):gsub("-+$", "")
  if selected == "" then
    selected = fallback or "workspace"
  end
  if #selected > 48 then
    selected = selected:sub(1, 48)
  end
  return selected
end
...
local function default_yq()
  return {
    available = function()
      ...
      local major = version:find("version v4", 1, true) or version:find("version 4", 1, true)
      local mike_farah = version:find("mikefarah", 1, true) ~= nil
      return result and result.code == 0 and mike_farah and major ~= nil
    end,
...
local function merge_context(left, right)
  ...
  for key, value in pairs(context_value(right)) do
    result[key] = value
  end
  return result
end
...
function Recorder:_close_body(body, remove)
  ...
  if file then
    self._pending_closes[file] = body.file and true or nil
  end
  if not called then
    return false, util.safe_message(closed)
  end
  return closed == true, err
end

function Recorder:_retry_closes()
  for file in pairs(self._pending_closes) do
...
function Recorder:_abandon(exchange)
  ...
  if not exchange.closed then
    self:_close_file(exchange.file)
  end
...
function Recorder:_write(exchange, data)
  ...
  if not written then
    self:_abandon(exchange)
    report(self, "failed to append exchange " .. exchange.id .. ": " .. tostring(err))
    return false
  end
...
function Recorder:_start(operation, request, supplied_context)
  ...
  if context.workspace ~= nil then
    assert(
      type(context.workspace) == "string" and context.workspace ~= "",
      "recording context workspace must be a non-empty string"
    )
    workspace = fs.canonical(context.workspace)
  end
  local sanitizer, recorded_request = sanitization.new(request, context, workspace, self._format)
  ...
  local provider = slug(selected_context.provider or selected_context.auth_method or "http")
  ...
  local group = workspace and day .. "-" .. slug(selected_context.session_id or "unscoped", "unscoped") or day
  ...
  local index_path = workspace_directory and fs.join(workspace_directory, "workspace.json") or nil
  ...
  local base =
    string.format("%s-%06d-%08d-%s-%s", filename_timestamp(now), vim.uv.os_getpid(), sequence, provider, origin)
  ...
  local first = {
    ...
    operation = operation,
    workspace = sanitizer.workspace and { root = sanitizer.workspace } or nil,
  ...
  local created, create_err = fs.atomic_replace(stage_path, encoded .. "\n", { mode = FILE_MODE })
  if not created then
...
function Recorder:_append_spooled_body(exchange, at_us)
  ...
  if not self:_write(exchange, header:sub(1, -2) .. ',"body":"') then
    return false
  end
...
function Recorder:_conversion_done(exchange, output)
  if output then
    ...
    if output.file then
      ...
      written, write_err = fs.atomic_replace(exchange.final_path, assert(output:text()), { mode = FILE_MODE })
    end
    if written then
      pcall(vim.uv.fs_unlink, exchange.stage_path)
      self:_published(exchange)
    else
      report(self, "failed to write YAML recording " .. exchange.id .. ": " .. tostring(write_err))
    end
  else
...
function Recorder:_retain(exchange)
  ...
  while true do
    ...
    if kind == "file" and path ~= exchange.final_path and final_recording_name(name) then
      local called, removed, remove_err, remove_code = pcall(vim.uv.fs_unlink, path)
      if not called or not removed and remove_code ~= "ENOENT" then
        report(
          self,
          "failed to remove a previous recording for " .. exchange.id .. ": " .. tostring(remove_err or removed)
        )
      end
    end
  end
...
function Recorder:_published(exchange)
  ...
  if not retained then
    report(self, "failed to retain rolling recording " .. exchange.id .. ": " .. tostring(retain_err))
  end
end
...
function Recorder:_publish(exchange)
  if self._format == "json" then
    ...
    if not moved then
      report(self, "failed to publish recording " .. exchange.id .. ": " .. tostring(err))
    else
      self:_published(exchange)
    end
    return
  ...
  local function done(success)
    ...
    settled = true
    local completed = pcall(self._conversion_done, self, exchange, success and output or nil)
    self:_close_body(output, false)
    self._pending_conversions = math.max(0, self._pending_conversions - 1)
    if not completed then
      report(self, "failed to publish converted recording " .. exchange.id)
    end
  end
  ---@param chunk string
  ---@return boolean
...
local function response_from(result, operation)
  if operation == "fetch" then
    ...
    if type(result.error) == "table" and type(result.error.response) == "table" then
...
function Recorder:_complete(exchange, result, operation)
  ...
  elseif operation == "request" and exchange.body.count == 0 then
    local body = type(response.body) == "string" and response.body
      or type(response.stdout) == "string" and response.stdout
      or nil
    if body then
      self:_chunk(exchange, body, settled_at)
    end
  end
  if exchange.finished then
    return
  end
  exchange.finished = true
  ...
  if not self:_append(exchange, completion) then
    return
  end
  ...
  if exchange.failed or not verified or not closed then
    report(self, "failed to close recording " .. exchange.id .. ": " .. tostring(close_err or "path identity changed"))
    return
  end
...
function Recorder:transport(base, context)
  ...
  local function wrap(operation, start)
    ...
    return function(opts)
      ...
      local recorded, exchange = pcall(recorder._start, recorder, operation, request, context)
      ...
      if operation == "request" then
        selected.on_chunk = function(chunk)
          ...
          if not ok then
            report(recorder, "failed to record a response chunk")
          end
          if on_chunk then
            on_chunk(chunk)
          end
      ...
      return async.run(function()
        ...
        if not settled then
          ...
          local ok = pcall(recorder._finish, recorder, exchange, failure, operation)
          if not ok then
            report(recorder, "failed to finish an exchange")
          end
...
function Recorder:destroy()
  ...
  local active = {}
  for exchange in pairs(self._exchanges) do
    active[#active + 1] = exchange
  end
  ...
  if self._pending_conversions > 0 then
    vim.wait(5000, function()
      return self._pending_conversions == 0
    end, 10)
  end
  self:_retry_closes()
  return true
...
function M.new(opts)
  opts = opts or {}
  local selected = opts.config or { enabled = false, format = "auto", retention = "rolling" }
  assert(type(selected) == "table", "recording configuration is required")
  if selected.enabled ~= true then
    return nil
  end
  ...
  assert(
    requested == "auto" or requested == "yaml" or requested == "json",
  ...
  local format = requested == "json" and "json" or requested == "yaml" and "yaml" or available and "yaml" or "json"
  if format == "yaml" and not available then
    return nil, util.error("configuration", "recording.format is yaml but a compatible yq v4 is unavailable")
  end
  local retention = selected.retention or "rolling"
  assert(retention == "rolling" or retention == "all", "recording retention must be rolling or all")
  return setmetatable({
    _format = format,
    _retention = retention,
    _directory = fs.normalize(opts.directory or selected.directory or vim.fn.stdpath("state") .. "/neoagent"),
